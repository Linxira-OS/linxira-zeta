...
function ensureSessionHeaderFirstLine(filePath: string): string {
  try {
    ...
    if (sessionIdx > 0) {
      const [sessionHeader] = lines.splice(sessionIdx, 1);
      lines.unshift(sessionHeader);
      const cleaned = lines.join("\n") + "\n";
      try {
        writeFileSync(filePath, cleaned, "utf8");
      } catch {
        const tempDir = join(tmpdir(), "pi-web-export-clean");
        mkdirSync(tempDir, { recursive: true });
        const tempFile = join(tempDir, `${randomUUID()}.jsonl`);
