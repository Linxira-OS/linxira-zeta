...
`~/.zeta/agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host omp doesn't route through the gateway). roboomp lives inside the omp
...
| `No API key found for <provider>` | `~/.zeta/agent/models.container.yml` mount missing or provider id mismatch with `ROBOMP_MODEL`. |
