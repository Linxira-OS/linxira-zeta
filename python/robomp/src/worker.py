...
def _stage_agent_home() -> None:
    ...
    for rel in (Path(".agent"), Path(".zeta/agent")):
    ...
    for root, dirs, files in os.walk(_AGENT_HOME):
        root_path = Path(root)
        if root_path == _AGENT_HOME / ".zeta":
            # ~/.zeta/run is slot-writable daemon presence state, not template
...
def _ensure_agent_run_dir() -> None:
    """Keep ``~/.zeta/run`` writable by every sandbox slot.

    omp registers daemon project presence under ``~/.zeta/run`` at startup,
...
def _run_rpc_blocking(
    ...
    *,
    ...
    # can resolve workspace packages (@linxiraos/pi-*) and actually run tests.
