...
(
   ...
   node -e "
		...
		pkg.overrides = {
			...
			'@linxiraos/snapcompact': '$snapcompact_tgz',
			'@linxiraos/pi-agent-core': '$agent_tgz',
			'@linxiraos/pi-tui': '$tui_tgz',
			'@linxiraos/omp-stats': '$stats_tgz',
			'@linxiraos/pi-coding-agent': '$coding_agent_tgz',
   ...
   [ "$omptype_probe" = "omp:false:tb" ] || {
      echo "Unexpected @linxiraos/omptype probe result: $omptype_probe"
      exit 1
   }
