#!/usr/bin/env bun
/**
 * Token-usage audit over the local omp session corpus (~/.zeta/agent/sessions/).
 *
 ...
 * asks a small model (default: anthropic/claude-sonnet-4-6 via @linxiraos/pi-ai)
 * to judge:
 ...
 * storage (~/.zeta/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
import { getAgentDbPath, isEnoent } from "@linxiraos/pi-utils";
import SYSTEM_PROMPT from "./audit-prompt.md" with { type: "text" };

const SESSIONS_ROOT = path.join(os.homedir(), ".zeta", "agent", "sessions");
const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
const CACHE_PATH = path.join(os.homedir(), ".zeta", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.zeta/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.zeta/stats-audit-cache.json)\n` +
