...
in
{
  options.programs.zeta = {
    ...
    settings = lib.mkOption {
      ...
      default = null;
      description = ''
        Settings written declaratively to {file}`~/.zeta/agent/config.yml`.
  ...
  config = lib.mkIf cfg.enable {
    ...
    home.activation.zetaConfig = lib.mkIf (cfg.settings != null) {
      before = [ ];
      after = [ "writeBoundary" ];
      data = ''
        run mkdir -p "$HOME/.zeta/agent"
        run install -m 600 ${configFile} "$HOME/.zeta/agent/config.yml"
      '';
    };
  };
}
