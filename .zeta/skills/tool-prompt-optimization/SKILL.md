...
`scripts/probe.ts` routes through `@linxiraos/pi-ai` (`completeSimple`) so model/auth/provider behavior matches production.

```bash
bun .zeta/skills/tool-prompt-optimization/scripts/probe.ts \
...
```bash
bun .zeta/skills/tool-prompt-optimization/scripts/probe-builtin.ts --tool <name> [--no-summary] [--show]
```
...
```ts
import { arkToWireSchema } from "@linxiraos/pi-ai"; // or toolWireSchema(tool)
