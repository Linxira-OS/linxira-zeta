...
`scripts/probe.ts` routes through `@zeta/pi-ai` (`completeSimple`) so model/auth/provider behavior matches production.
...
```ts
import { arkToWireSchema } from "@zeta/pi-ai"; // or toolWireSchema(tool)
