...
function loadVt() {
	...
	vtApi ??= import("kitty-vt-wasm").catch((error) => {
		vtApi = null;
		throw new Error(
			`screen emulation needs kitty-vt-wasm — run \`bun install\` in .zeta/tools (${error})`,
		);
	});
	return vtApi;
}
...
async function loadCanvas() {
	...
	try {
		...
		throw new Error(
			`shot needs @napi-rs/canvas — run \`bun install\` in .zeta/tools (${error})`,
		);
	}
...
const factory = (omp: ToolHost) => {
	const startSession = async (params: TuiParams): Promise<string> => {
		...
		// binding is assigned before the first chunk can arrive. That forward
		// reference is why this stays `let` despite the single assignment.
