...
`,
	...
	it("returns a nonzero exit when wait-for reaches its bound", () => {
		dir = createTempDir();
		const result = spawnSync(BINARY, ["--size", "80x24", "--exec", `wait-for "NEVER VISIBLE" timeout=40`], {
			encoding: "utf8",
			timeout: 15000,
			env: { ...process.env, TTT_CONFIG_DIR: join(dir, "config") },
		});
	...
	it("rejects an overflowing wait-for timeout as invalid input", () => {
		dir = createTempDir();
		const result = spawnSync(BINARY, ["--size", "40x12", "--exec", "wait-for x timeout=9223372036854775807"], {
			encoding: "utf8",
			timeout: 15000,
			env: { ...process.env, TTT_CONFIG_DIR: join(dir, "config") },
		});
