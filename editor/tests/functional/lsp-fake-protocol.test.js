...
const LIFECYCLE_METHODS = new Set(["initialize", "initialized", "textDocument/didOpen", "textDocument/didChange"]);
...
function clientMessages(trace) {
	return trace.filter(entry => entry.direction === "client->server").map(entry => entry.message);
}
...
function traceEntriesFor(trace, direction, method) {
	return trace.filter(entry => entry.direction === direction && entry.message.method === method);
}
...
function assertRequestAfterDocument(trace, request, text) {
	const requestEntry = trace.find(entry => entry.direction === "client->server" && entry.message.id === request.id);
...
function assertProtocol(trace, file, expectedTexts) {
	...
	expect(trace.map(entry => entry.sequence)).toEqual(Array.from({ length: trace.length }, (_, index) => index + 1));
	...
	expect(messagesFor(trace, "initialized")).toEqual([{ jsonrpc: "2.0", method: "initialized", params: {} }]);
	...
	expect(changes.map(message => message.params.contentChanges)).toEqual(expectedTexts.map(text => [{ text }]));
...
describe("deterministic fake LSP protocol", () => {
	...
	it("uses byte-length framing in both directions for a multibyte dot completion", () => {
		...
		expect(
			trace.find(entry => entry.direction === "server->client" && entry.message.id === completions[0].id).message,
		).toEqual({
			jsonrpc: "2.0",
			id: completions[0].id,
			result: {
	...
	it("accepts a dot completion without duplicating the trigger character", () => {
		...
		assertProtocol(trace, file, [...expectedDocumentTexts("console."), `${INITIAL_TEXT}console.log`]);
		const completions = messagesFor(trace, "textDocument/completion");
		expect(completions).toEqual([
			{
				jsonrpc: "2.0",
