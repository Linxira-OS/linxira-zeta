...
describe("find navigation (F3 / Shift+F3)", () => {
	it("F3 cycles through matches", () => {
		dir = createTempDir();
		const file = createTempFile(dir, "cycle.txt", "cat here\ndog there\ncat again\nbird\ncat end");
	...
	it("Shift+F3 goes to previous match", () => {
		dir = createTempDir();
		const file = createTempFile(dir, "prev.txt", "cat here\ndog there\ncat again\nbird\ncat end");
	...
	it("F3 wraps around from last match to first", () => {
		dir = createTempDir();
		const file = createTempFile(dir, "wrap.txt", "cat start\ndog middle\ncat end");
	...
	it("find with no matches shows zero count", () => {
		dir = createTempDir();
		const file = createTempFile(dir, "nomatch.txt", "cat here\ndog there\nbird end");
