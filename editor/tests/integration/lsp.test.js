...
import { cpSync, readFileSync, readdirSync, unlinkSync, existsSync } from "node:fs";
...
function lspServerAvailable(fixtureDir) {
	const settings = JSON.parse(readFileSync(resolve(fixtureDir, "settings.json"), "utf8"));
...
function prepareFixture(fixtureDir) {
	...
	const files = readdirSync(dir).filter(file => !["spec.json", "settings.json", "install.sh"].includes(file));
	return {
