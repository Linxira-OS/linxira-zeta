...
import { copyFileSync, cpSync, existsSync, readFileSync, statSync, unlinkSync, writeFileSync } from "node:fs";
...
const LOG_FILE = resolve(process.env.HOME || process.env.USERPROFILE, ".config", "ttt", "ttt.log");
...
function prepareCase(language) {
	...
	const spec = JSON.parse(readFileSync(resolve(fixtureDir, "spec.json"), "utf8"));
	const dir = createTempDir();
	cpSync(fixtureDir, dir, { recursive: true });
	const sourceName = { c: "test.c", go: "test.go", typescript: "test.ts" }[language];
	const source = resolve(dir, sourceName);
	const extension = extname(source);
	const signature = resolve(dir, `signature${extension}`);
	copyFileSync(source, signature);
	writeFileSync(signature, `${readFileSync(signature, "utf8")}\n// TTT_SIGNATURE_COPY\n`, "utf8");
	...
	if (language === "typescript") {
		...
		writeFileSync(completion, `${readFileSync(completion, "utf8")}\n// TTT_COMPLETION_COPY\n`, "utf8");
	}
...
describe("pinned real-LSP compatibility", () => {
	...
	for (const language of LANGUAGES) {
		...
		testFn(`${language} completes one server lifecycle`, async () => {
			...
			const signatureReady = waitForLogAfter("lsp diagnostics.*path=.*signature", signatureOpenMark);
			...
			const signatureLog = waitForLogAfter(spec.signatureHelp.log, signatureMark);
			expect(signatureLog).toMatch(new RegExp(spec.signatureHelp.log));

			tui.press("escape");
			const completionOpenMark = logSize();
			await previousTab(port, language === "typescript" ? "TypeScript completion smoke" : "TTT_COMPLETION_COPY");
			const completionReady = waitForLogAfter("lsp diagnostics.*path=.*completion", completionOpenMark);
			expect(completionReady).toMatch(/lsp diagnostics.*path=.*completion/);
			const completionMark = logSize();
			if (language === "typescript") {
				navigateTo("2:0");
				tui.type("console.");
				const completionLog = waitForLogAfter("lsp completion response.*count=[1-9]", completionMark);
				tui.waitStable();
				...
				const completionLog = waitForLogAfter(spec.completion.log, completionMark);
