...
- **Manual repro (interactive):** scratch file with a repeated word:
   ...
   2. `Ctrl+K` then `L` (`multicursor.selectAll`) → all three `foo` selected, status bar "3 cursors". _(This step is not the bug — it is the normal way to get multiple cursors.)_
...
- **Repro:** fold an inner `if true {` block, insert a blank line at the top of the file → the fold marker now collapses the _enclosing function's_ body instead.
