...
import { discoverAuthStorage } from "@linxiraos/pi-ai/auth-broker/discover";
...
async function fetchAntigravityModels(): Promise<ModelSpec<"google-gemini-cli">[]> {
	...
	if (!access) {
		...
		console.log("Tip: If you are logged in under a specific profile, run with ZETA_PROFILE=<name>.");
		return [];
	}
	try {
