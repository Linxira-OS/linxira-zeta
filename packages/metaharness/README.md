...
1. **Local zeta, not npm.** By default the runner bind-mounts the repo
   read-only into each task container (`--install source`) and runs zeta
   ...
   `/opt/zeta/bin` — so trial setup needs zero outbound network. Alternatives:
   `--install local` (pack a tarball per run) or `--binary` (prebuilt
   `dist/zeta-linux-*` self-contained binaries).
...
image as an x86_64 KVM microVM on a Vibemon host, streams zeta's `--mode rpc`
...
broker for its lifetime, and reverse-tunnels the loopback zeta auth gateway so
...
| `--env <KEY[=VALUE]>` | — | Forward env into the zeta container (repeatable); `KEY` alone forwards the host value |
| `--binary <path>` | — | Prebuilt zeta binary (repeat for arm64+x64) |
| `--install <source\|local\|published>` | `source` | `source` = repo bind-mount, `local` = tarball pack, `published` = npm `@linxiraos/zeta` |
...
arc; ~$0.001 per report). API keys resolve through zeta's auth storage.
