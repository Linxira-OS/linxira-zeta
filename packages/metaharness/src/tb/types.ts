 ...
 * zeta runs inside that guest over raw RPC stdio carried by the Vibemon SDK's
 ...
 * - `agent.ts` — zeta Linux binary build + guest install
...
/** Guest CPU architecture, normalized to zeta binary naming. */
...
/** OpenRouter request-routing suffix applied by zeta. */
...
export interface GatewayConfig {
	...
	url: string;
	/** Gateway bearer (`zeta auth-gateway token`); `"no-auth"` when the gateway runs open. */
	token: string;
...
/** Host paths of self-contained zeta Linux binaries, keyed by guest architecture. */
export interface AgentBinaries {
	x64?: string;
	arm64?: string;
	/** zeta package version the binaries were built from (for run provenance). */
	version: string;
}
