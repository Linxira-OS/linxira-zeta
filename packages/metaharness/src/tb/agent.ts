...
async function refreshCrossNatives(arches: GuestArch[], version: string): Promise<void> {
	for (const arch of arches) {
		...
		const response = await fetch(`https://registry.npmjs.org/@linxiraos/${pkg}/-/${pkg}-${version}.tgz`);
		if (!response.ok) throw new Error(`Fetching @linxiraos/${pkg}@${version} failed: HTTP ${response.status}`);
		...
		if (extracted === 0) throw new Error(`@linxiraos/${pkg}@${version} tarball contained no .node files`);
	}
}

/** Build and cache self-contained zeta binaries for the selected guest architectures. */
export async function prepareAgentBinaries(opts: {
	...
	for (const arch of arches) {
		cached[arch] = path.join(opts.cacheDir, `zeta-linux-${arch}-${manifest.version}`);
	}
	...
	if (missing.length > 0) {
		...
		for (const arch of missing) {
			const source = path.join(CODING_AGENT_DIR, "binaries", `zeta-linux-${arch}`);
...
export async function installAgent(vm: TrialVm, binaries: AgentBinaries, gateway: GatewayConfig): Promise<string> {
	...
	const entrypoint = "/opt/zeta/zeta";
	const mkdir = await vm.exec("mkdir -p /opt/zeta");
	if (mkdir.exitCode !== 0) throw new Error(`Could not create /opt/zeta: ${mkdir.stderr.trim()}`);
	await vm.copyTo(binary, entrypoint);
	const chmod = await vm.exec(`chmod 755 ${shellQuote(entrypoint)}`);
	if (chmod.exitCode !== 0) throw new Error(`Could not make zeta executable: ${chmod.stderr.trim()}`);
	const warmup = await vm.exec(`${shellQuote(entrypoint)} --version`);
	if (warmup.exitCode !== 0) throw new Error(`zeta --version failed: ${warmup.stderr.trim()}`);
...
`;
	...
	if (writeConfig.exitCode !== 0)
		throw new Error(`Could not install zeta configuration: ${writeConfig.stderr.trim()}`);
	return entrypoint;
}
