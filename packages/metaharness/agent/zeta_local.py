...
class OmpLocal(BaseInstalledAgent):
    ...
    async def _install_published(self, environment: BaseEnvironment) -> str:
        app = f"{self._home}/.omp-bench/app"
        spec = f"@linxiraos/pi-coding-agent@{self._pkg_version}"
        await self.exec_as_agent(
            environment,
            command=self._wrap(
                "set -e; "
        ...
        return f"{app}/node_modules/@linxiraos/pi-coding-agent/dist/cli.js"
    ...
    async def _write_config(self, environment: BaseEnvironment) -> None:
        """Write $HOME/ .zeta/agent/config.yml: the web_search and find toggles.
