...
import { getNextTimeBasedPricingTransition } from "@linxiraos/pi-catalog/models";
import type { Model, ModelCost } from "@linxiraos/pi-catalog/types";
...
/** Whether the right-hand sidebar is currently open (host app sets it; hides the session-name segment). */
let statusLineSidebarOpen = false;

/** Set from the host app's `tui.sidebar` setting; the open sidebar already shows the session identity. */
export function setStatusLineSidebarOpen(open: boolean): void {
	statusLineSidebarOpen = open;
}
...
export class StatusLineComponent<TSession extends StatusLineSession = StatusLineSession> implements Component {
	...
	getSidebarContext(width: number): SegmentContext {
		return this.#buildSegmentContext(width, this.#resolveSettings().segmentOptions, false, true, true, Date.now());
	}
	...
	#renderStatusLine(
		...
		for (const segId of leftSegmentIds) {
			...
			if ((layout === "band" || statusLineSidebarOpen) && segId === "session_name") continue;
