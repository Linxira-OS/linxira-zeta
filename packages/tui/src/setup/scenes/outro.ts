...
export function renderSetupOutro(width: number, height: number, elapsedMs: number): string[] {
	...
	const logo = gradientLogo(ZETA_LOGO, progress * 1.2, { pos: (progress * 2) % 1, strength: 1 - progress });
	const title = theme.bold(
		theme.fg("success", `${theme.status.success} ${tuiText("setupOutroTitle", "Setup saved")}`),
	);
	const subtitle = theme.fg("muted", tuiText("setupOutroSubtitle", "Handing off to the normal CLI…"));
