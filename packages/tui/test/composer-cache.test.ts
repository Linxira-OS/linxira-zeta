...
import { COMPOSER_DEFAULTS, type ComposerStatusSnapshot } from "@linxiraos/pi-tui/prompt/composer";
import {
...
describe("composer startup cache", () => {
	...
	it("loads XDG_CACHE_HOME from the home .env before the first cache access", async () => {
		...
		try {
			...
			const composerCacheModule = Bun.resolveSync("@linxiraos/pi-tui/prompt/composer-cache", import.meta.dir);
			const script = [
				'import * as path from "node:path";',
