...
import { $ } from "bun";
import { getDefaultPasteImageKeys } from "@linxiraos/pi-tui/app-keybindings";
import {
	chipLabel,
...
} from "@linxiraos/pi-tui/prompt/custom-editor";
import {
	SPACE_HOLD_MECHANICAL_RUN,
	SPACE_HOLD_RELEASE_MS,
	SPACE_REPEAT_MAX_GAP_MS,
} from "@linxiraos/pi-tui/space-hold";
...
async function decorateInFreshProcess(text: string, imageLinks?: readonly string[]): Promise<string> {
	const customEditorUrl = import.meta.resolve("@linxiraos/pi-tui/prompt/custom-editor");
	const script = `
