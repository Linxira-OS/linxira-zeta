...
import { KeybindingsManager as AppKeybindingsManager, setKeyHintPlatform } from "@linxiraos/pi-tui/app-keybindings";
import type { ModelBrowserItem } from "@linxiraos/pi-tui/overlays/model-browser";
import { setInternalUrlCompletionHost } from "@linxiraos/pi-tui/prompt/internal-url-autocomplete";
import { createPromptActionAutocompleteProvider } from "@linxiraos/pi-tui/prompt/prompt-action-autocomplete";
import { getSelectListTheme, initTheme, theme } from "@linxiraos/pi-tui/theme";
...
describe("prompt action autocomplete", () => {
	...
	it("falls through to internal-url completion for allowArgs commands without argument completions", async () => {
		...
		expect(suggestions).not.toBeNull();
		expect(suggestions?.prefix).toBe("zeta://");
