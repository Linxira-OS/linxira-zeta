...
import { getSubprocessToolRenderer } from "@linxiraos/pi-tui/tools/subprocess";
import type { AgentProgress, SingleResult, TaskToolDetails } from "@linxiraos/pi-tui/tools/task";
import { FEED_MODEL_BADGE_WIDTH } from "@linxiraos/pi-tui/render/render-utils";
...
describe("task progress rendering", () => {
	...
	it("preserves long descriptions below bounded IDs and required status with badges on or off", async () => {
		...
		for (const enabled of [true, false]) {
			setFeedModelBadgeEnabled(enabled);
