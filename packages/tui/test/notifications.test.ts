...
import * as desktopNotify from "@linxiraos/pi-tui/desktop-notify";
...
describe("terminal notifications", () => {
	...
	it("formats structured Kitty OSC 99 title and body chunks", () => {
		...
		expect(out).toBe(
			"\x1b]99;i=complete-1:f=WmV0YQ==:a=focus:u=1:t=Y29tcGxldGlvbg==:n=aW5mbw==:s=aW5mbw==:w=5000:d=0;Session\x1b\\" +
	...
	it("base64-encodes unsafe OSC 99 payload controls", () => {
		...
		expect(out).toBe("\x1b]99;i=unsafe:f=WmV0YQ==:e=1;TGluZSAxCkxpbmUgMg==\x1b\\");
	});
	...
	it("queries and confirms OSC 99 support before rich notifications", () => {
		...
		try {
			const query = writes.find(w => w.startsWith("\x1b]99;i=zeta-probe-") && w.endsWith("\x1b\\\x1b[c"));
