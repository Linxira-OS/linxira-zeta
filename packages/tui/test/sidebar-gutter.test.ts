...
describe("TUI sidebar gutter", () => {
	it("paints the main area left and the gutter right; committed history carries no gutter text", async () => {
		...
		try {
			...
			main.append(5);
			tui.requestRender();
			await term.waitForRender(() => term.getScrollBuffer().some(line => line.includes("MAIN-060")));
			await new Promise(resolve => setTimeout(resolve, 50));

			// getScrollBuffer() = committed history + live viewport rows; the
			// viewport legitimately shows the gutter, so assert history only.
			const history = term.getScrollBuffer().slice(0, -term.rows);
			expect(history.length).toBeGreaterThan(0);
			for (const line of history) {
				expect(line).not.toContain("GTR");
			}
			expect(history.some(line => line.includes("MAIN-040"))).toBe(true);
		} finally {
			tui.stop();
		}
	});
