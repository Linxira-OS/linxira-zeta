...
describe("worker-host selectors", () => {
	it("recognizes the shared selector namespace without claiming ordinary CLI arguments", () => {
		expect(WORKER_HOST_SELECTOR_PREFIX).toBe("__zeta_worker_");
		expect(isWorkerHostSelector("__zeta_worker_stats_sync")).toBeTrue();
		expect(isWorkerHostSelector("__zeta_worker_computer")).toBeTrue();
