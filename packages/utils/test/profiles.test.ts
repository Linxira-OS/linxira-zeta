...
describe("profile directories", () => {
	...
	beforeEach(async () => {
		...
		// `ZETA_CODING_AGENT_DIR` baseline matching the env we just configured.
	...
	it("keeps XDG-backed named profile state under profile-specific roots", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "zeta", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "zeta", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "zeta", "profiles", "work"), { recursive: true });

		setProfile("work");

		expect(getAgentDbPath()).toBe(path.join(process.env.XDG_DATA_HOME, "zeta", "profiles", "work", "agent.db"));
		expect(getSessionsDir()).toBe(path.join(process.env.XDG_DATA_HOME, "zeta", "profiles", "work", "sessions"));
		expect(getPythonGatewayDir()).toBe(
			path.join(process.env.XDG_STATE_HOME, "zeta", "profiles", "work", "python-gateway"),
		);
	});
	...
	it("does not silently switch a named profile to XDG once the base app dir appears", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "zeta"), { recursive: true });
	...
	it("rejects path-like profile names", () => {
		expect(() => setProfile("../work")).toThrow("Invalid Zeta profile");
		expect(() => setProfile("work/team")).toThrow("Invalid Zeta profile");
	});
	...
	it("restores the pre-profile ZETA_CODING_AGENT_DIR override on reset", () => {
		const customAgentDir = path.join(tempRoot, "custom-agent");
		setAgentDir(customAgentDir);
		expect(getAgentDir()).toBe(customAgentDir);
		expect(process.env.ZETA_CODING_AGENT_DIR).toBe(customAgentDir);
	...
	it("clears ZETA_CODING_AGENT_DIR on reset when nothing was set originally", () => {
		delete process.env.ZETA_CODING_AGENT_DIR;
		...
		setProfile("work");
		setProfile(undefined);
		expect(process.env.ZETA_CODING_AGENT_DIR).toBeUndefined();
	});
	...
	it("does not restore a profile-derived agent dir as the default baseline", () => {
		// Reproduces a child process that inherited ZETA_PROFILE=work plus the
		// profile-derived ZETA_CODING_AGENT_DIR that setProfile propagates to
		...
		expect(process.env.ZETA_CODING_AGENT_DIR).toBe(workAgentDir);

		// Re-snapshot exactly as module load would, now that ZETA_PROFILE and the
		// profile-derived ZETA_CODING_AGENT_DIR are present in the environment.
		__resetProfileSnapshotForTests();
...
describe("profile env + name validation", () => {
	it("resolves the single ZETA_PROFILE key and treats empty/default as the default profile", () => {
		expect(resolveProfileEnv("work")).toBe("work");
		// An explicitly-empty ZETA_PROFILE selects the default profile.
	...
	it("rejects uppercase profile names so isolation is filesystem-independent", () => {
		...
		expect(() => normalizeProfileName("WORK")).toThrow("Invalid Zeta profile");
		expect(() => normalizeProfileName("Work")).toThrow("Invalid Zeta profile");
...
describe("dirs module import behavior", () => {
	...
	it("exposes worker-host without loading agent env", async () => {
		...
		try {
			const workerHostUrl = import.meta.resolve("@linxiraos/pi-utils/worker-host");
	...
	it("ignores inherited profile agent dir when ZETA_PROFILE explicitly selects default", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "pi-utils-dirs-default-profile-"));
		const probeConfigDir = `.zeta-default-profile-${Snowflake.next()}`;
		try {
			const dirsUrl = url.pathToFileURL(path.join(import.meta.dir, "..", "src", "dirs.ts")).href;
			...
			for (const zetaProfile of ["", "default"]) {
				const probePath = path.join(root, `default-profile-${zetaProfile || "empty"}.ts`);
				await Bun.write(
					probePath,
					[
	...
	it("honors XDG dir keys from a profile .env applied after the resolver froze", async () => {
		...
		const profileConfigDir = `.zeta-env-xdg-${Snowflake.next()}`;
		try {
			...
			// Named profiles only adopt XDG when their own XDG path already exists.
			const xdgProfileRoot = path.join(xdgStateRoot, "zeta", "profiles", "work");
