...
export async function embedNativeAddon({
	...
	for (const addon of available) {
		...
		if (!containsVersionSentinel(bytes, versionSentinel)) {
			throw new Error(
				`Native addon ${addon.path} does not contain the @linxiraos/pi-natives@${version} version sentinel ` +
					`\`${versionSentinel}\`. Rebuild it or fetch @linxiraos/pi-natives-${platformTag}@${version} before embedding.`,
			);
		}
