...
- `src/lib/` — vendored wire codec (`codec.ts` AES-256-GCM, `link.ts` envelope + link grammar), `socket.ts` reconnecting relay socket, `client.ts` guest session store (`GuestClient` + immutable snapshots for `useSyncExternalStore`). Shared protocol shapes come from `@linxiraos/pi-wire`.
...
The package is intentionally standalone — no dependency on `@linxiraos/zeta` at runtime or type level. Wire-shape drift is prevented by consuming the same `@linxiraos/pi-wire` contracts as the host, with sealed-frame interop still covered by `test/codec.test.ts`.
