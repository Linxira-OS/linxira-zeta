 ...
 * `BlobStore` keep writing to `~/.zeta/agent/...`. Reach for an object store
 * (S3, R2, GCS) if you need those off-host too.
 */

import { createAgentSession, RedisSessionStorage, SessionManager } from "@linxiraos/zeta";
...
const storage = await RedisSessionStorage.create({
	client: redis,
	prefix: "zeta:sessions:", // optional, this is the default
});
