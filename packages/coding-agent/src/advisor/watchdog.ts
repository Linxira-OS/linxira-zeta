...
import { CONFIG_DIR_NAME, getAgentDir, isEnoent, logger, prompt } from "@linxiraos/pi-utils";
 ...
 * `.zeta/<F>` for each given filename — and return the readable candidates with
...
export async function collectConfigCandidates(
	...
	// 1. User level: ~/.zeta/<F> (or active profile agent dir)
	if (resolvedAgentDir) {
	...
	// 2. Project levels (both standalone and native config .zeta/): walk up from cwd to repoRoot / home
	let current = cwd;
	while (true) {
		for (const filename of filenames) {
