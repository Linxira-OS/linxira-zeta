/**
 * Web Gateway skill-lock integration.
 *
 * Semantic port of `web-ui/lib/skill-lock.ts`. The `skills` CLI writes
 * `.skill-lock.json` entries (global: `~/.agents/.skill-lock.json`, project:
 * `<cwd>/skills-lock.json`); this module annotates skill listings with the
 * install info needed by the web-ui skills UI.
 */

import { existsSync, readFileSync } from "node:fs";
import { homedir } from "node:os";
import { isAbsolute, join, relative, resolve, sep } from "node:path";
import type { SkillInfo, SkillInstallInfo, SkillInstallScope } from "./dto";
