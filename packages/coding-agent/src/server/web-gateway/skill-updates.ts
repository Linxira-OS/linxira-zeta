/**
 * Web Gateway skill-update checks.
 *
 * Semantic port of `web-ui/lib/skill-updates.ts`: global skills are checked
 * against the GitHub tree hash of their source repo, project skills against
 * the skills.sh snapshot hash. All network work is done in-process so the
 * check runs inside the gateway, not the web-ui process.
 */
...
import type { SkillInstallInfo, SkillUpdateResult } from "./dto";
...
/** Bridge the DOM `Response` used here onto Bun's fetch result type. */
const defaultFetcher: Fetcher = (input, init) => fetch(input, init) as unknown as Promise<Response>;
...
async function fetchJson(url: string, fetcher: Fetcher, headers?: Record<string, string>): Promise<unknown> {
...
async function checkGlobalSkill(
	...
	try {
		...
		if (folder && Array.isArray(raw.tree)) {
			const entry = (raw.tree as GitHubTreeEntry[]).find(item => item.type === "tree" && item.path === folder);
...
export async function checkSkillUpdates(
	...
	const cachedFetcher: Fetcher = async (input, init) => {
		...
		return (await request).clone() as unknown as Response;
	};
