 ...
 * (`~/.zeta/skillshare/@scope/name/<version>/`). Project locks are found by
 * walking up from cwd like native `.zeta/skills` (closest first); the user lock
 * lives in the agent dir. Priority 95 sits just below native (100) so authored
 * skills win name collisions.
 */
import * as fs from "node:fs/promises";
import * as path from "node:path";
import { getAgentDir, isEnoent, logger, parseFrontmatter } from "@linxiraos/pi-utils";
...
async function loadLockedSkill(
	...
	// sits behind a symlink, e.g. a dotfiles-managed ~/.zeta.
