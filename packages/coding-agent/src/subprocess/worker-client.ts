...
export function resolveExecutablePath(): string {
	...
	if (isCompiledBinary() && !isExecutable(executable)) {
		...
		const candidates = [
			...
			// Generic fallback to finding "zeta" on PATH
			$which("zeta", { requireAbsolutePaths: true, cache: WhichCachePolicy.Bypass }),
		];
 ...
 * runtime. The ONNX addons installed on demand under `~/.zeta/agent/cache/**`
