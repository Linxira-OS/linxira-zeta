...
export interface TransformersRuntimeMetadata {
	__zetaRuntimeNodeModules?: string;
	__zetaTransformersEntry?: string;
	__zetaCudaRepairError?: string;
}
...
function attachTransformersRuntimeMetadata<T extends ConfigurableTransformers>(
	...
	runtime.__zetaRuntimeNodeModules = metadata.__zetaRuntimeNodeModules;
	runtime.__zetaTransformersEntry = metadata.__zetaTransformersEntry;
	runtime.__zetaCudaRepairError = metadata.__zetaCudaRepairError;
	return runtime;
}
...
function cudaFailureCause(
	...
	if (metadata.__zetaCudaRepairError) {
		return `ONNX Runtime CUDA provider install failed: ${metadata.__zetaCudaRepairError}`;
	}
...
function resolveOnnxRuntimePackageDir(metadata: TransformersRuntimeMetadata): string | null {
	const entry = metadata.__zetaTransformersEntry;
	if (entry) {
		try {
	...
	const nodeModules = metadata.__zetaRuntimeNodeModules;
...
export async function formatOnnxRuntimeCudaDiagnostics(
	...
	const sideRuntime = metadata.__zetaRuntimeNodeModules;
	const lines = [
		"ONNX Runtime CUDA diagnostics:",
		`  PI_TINY_DEVICE=${requestedDevice} requested CUDAExecutionProvider`,
...
export function loadTransformersRuntime<T extends ConfigurableTransformers, K>(
	...
	return holder.load(async () => {
		...
		return attachTransformersRuntimeMetadata(configureTransformers(require_(entry) as T), {
			__zetaRuntimeNodeModules: path.join(installedDir, "node_modules"),
			__zetaTransformersEntry: entry,
			__zetaCudaRepairError: cudaRepairError,
		});
	});
}
