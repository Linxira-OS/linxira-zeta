...
#
# `__zeta_emit_referenced_exports` reads captured function bodies from stdin,
...
__zeta_sq_quote() {
	...
	__zeta_qout=
	__zeta_sq=\'
	while case "$__zeta_qbuf" in *$__zeta_sq*) true ;; *) false ;; esac; do
		__zeta_qout=$__zeta_qout${__zeta_qbuf%%"$__zeta_sq"*}"'\\''"
		__zeta_qbuf=${__zeta_qbuf#*"$__zeta_sq"}
	done
	__zeta_qout=$__zeta_qout$__zeta_qbuf
	printf "'%s'" "$__zeta_qout"
}
...
__zeta_emit_export_for() {
	...
	__zeta_xv=
	eval "__zeta_xv=\"\${$1}\"" 2>/dev/null || return
	printf 'export %s=%s\n' "$1" "$(__zeta_sq_quote "$__zeta_xv")"
}
...
__zeta_emit_referenced_exports() {
	grep -oE '\$\{?[A-Za-z_][A-Za-z0-9_]*' \
		| sed -E 's/^\$\{?//' \
		| sort -u \
		| while IFS= read -r __zeta_name; do
			__zeta_emit_export_for "$__zeta_name"
		done
}
