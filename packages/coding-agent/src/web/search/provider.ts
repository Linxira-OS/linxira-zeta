...
import { SEARCH_PROVIDER_LABELS, type SearchProviderId } from "@linxiraos/pi-tui/tools/web-search";
...
const PROVIDER_META: Record<SearchProviderId, ProviderMeta> = {
	openrouter: {
		...
		load: async () => new (await import("./providers/openrouter")).OpenRouterGroundedProvider(),
	},
	bing: {
		id: "bing",
 ...
 */
export async function getSearchProvider(id: string): Promise<SearchProvider> {
	const cached = instanceCache.get(id as SearchProviderId);
	if (cached) return cached;
	const meta = isSearchProviderId(id) ? PROVIDER_META[id] : undefined;
	if (!meta) {
		throw new Error(`Unknown search provider: ${id}`);
	}
	const provider = await meta.load();
	instanceCache.set(id as SearchProviderId, provider);
	return provider;
}
