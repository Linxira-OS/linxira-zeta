...
const LSP_STATUS_LABELS: Record<string, string> = {
	ready: M.ccLspStatusReady,
	available: M.ccLspStatusAvailable,
	connecting: M.ccLspStatusConnecting,
	error: M.ccLspStatusError,
};
...
export class CommandController {
	...
	async handleDumpCommand(): Promise<void> {
		try {
			...
			try {
				...
				sidecarError = error instanceof Error ? error.message : M.ccUnknownError;
			}
	...
	handleAdvisorDumpCommand(isRaw = false) {
		try {
			...
			this.ctx.showError(
				M.ccFailedToCopyAdvisorHistoryFmt.replace("%s", error instanceof Error ? error.message : M.ccUnknownError),
			);
		}
	}
	...
	async handleDebugTranscriptCommand(): Promise<void> {
		try {
			...
			this.ctx.showError(
				M.ccFailedToWriteDebugTranscriptFmt.replace(
					"%s",
					error instanceof Error ? error.message : M.ccUnknownError,
				),
			);
		}
	}
		...
		if (customShare) {
			...
			try {
				...
				if (typeof result === "string") {
					...
					const parts: string[] = [];
					if (result.url) parts.push(M.ccShareUrlFmt.replace("%s", result.url));
	...
	async handleSessionCommand(): Promise<void> {
		...
		let info = `${theme.bold(M.ccSessionInfoTitle)}\n\n`;
			...
			for (const server of this.ctx.lspServers) {
				...
				const statusText =
					server.status === "error" && server.error
						? `${M.ccLspStatusError}: ${server.error}`
						: (LSP_STATUS_LABELS[server.status] ?? server.status);
		...
		if (this.ctx.mcpManager) {
			...
			if (mcpServers.length === 0) {
				...
				for (const name of mcpServers) {
					...
					info += `${theme.fg("dim", `${name}:`)} ${theme.fg("success", "connected")} ${theme.fg("dim", M.ccToolsCountFmt.replace("%s", String(toolCount)))}\n`;
				}
			}
		}
	...
	static readonly #advisorStatusLabel: Record<string, string> = {
		running: M.ccAdvisorStatusRunning,
		paused: M.ccAdvisorStatusPaused,
		no_model: M.ccAdvisorStatusNoModel,
		quota_exhausted: M.ccAdvisorStatusQuotaExhausted,
		error: M.ccAdvisorStatusError,
	};
	...
	async #mmShow(state: HindsightSessionState, id: string): Promise<void> {
		try {
			...
			const tags =
				model.tags && model.tags.length > 0 ? `\n${M.ccTagsLineFmt.replace("%s", model.tags.join(", "))}` : "";
			const refreshed = model.last_refreshed_at
				? `\n${M.ccLastRefreshedLineFmt.replace("%s", model.last_refreshed_at)}`
				: "";
	...
	async #mmSeed(state: HindsightSessionState): Promise<void> {
		try {
			...
			if (seeds.length === 0) {
				this.ctx.showStatus(M.ccNoSeedsForScopeFmt.replace("%s", config.scoping));
				return;
			}
	...
	async handleDropCommand(): Promise<void> {
		if (!this.ctx.sessionManager.getSessionFile()) {
			this.ctx.showError(M.ccNothingToDrop);
			return;
		}
		await this.#runNewSessionFlow({ drop: true }, M.ccSessionDropped);
	}
	...
	async handleForkCommand(): Promise<void> {
		...
		const shortPath = sessionFile?.split("/").pop() ?? M.ccNewSession;
		this.ctx.present([
			new Spacer(1),
			new Text(
	...
	async handleMoveCommand(targetPath?: string): Promise<void> {
		...
		this.ctx.present([
			new Spacer(1),
			new Text(
				`${theme.fg("accent", `${theme.status.success} ${M.ccMovedToFmt.replace("%s", resolvedPath)}`)}`,
				1,
				1,
			),
		]);
	...
	async handleHandoffCommand(customInstructions?: string): Promise<void> {
		...
		try {
			...
			this.ctx.present([
				new Spacer(1),
				new Text(`${theme.fg("accent", `${theme.status.success} ${M.ccNewSessionWithHandoff}`)}`, 1, 1),
			]);
			if (result.savedPath) {
...
function resolveProviderAuthMode(authStorage: AuthStorage, provider: string): string {
	...
	if (authStorage.has(provider)) {
		return M.ccAuthModeApiKey;
	}
	if (getEnvApiKey(provider)) {
		return M.ccAuthModeEnvApiKey;
	}
	if (authStorage.hasAuth(provider)) {
		return M.ccAuthModeRuntimeFallback;
	}
	return M.ccAuthModeUnknown;
}
...
function formatAggregateAmount(limits: UsageLimit[]): string {
	...
	if (uniqueAccountIds.size > 0)
		return (uniqueAccountIds.size === 1 ? M.ccAcctSingularFmt : M.ccAcctPluralFmt).replace(
			"%s",
			String(uniqueAccountIds.size),
		);
...
export function formatCompactQuota(
	...
	for (const { limit, fraction } of entries) {
		...
		const parts = [M.ccPctUsedFmt.replace("%s", header).replace("%s", String(pct))];
		const reset = resolveResetRange([limit], nowMs);
		if (reset) parts.push(reset);
		lines.push(parts.join(" · "));
	}
	return M.ccQuotaLineFmt.replace("%s", lines.join(" │ "));
}
...
export function renderUsageReports(
	...
	const headerSuffix = latestFetchedAt
		? ` ${M.ccAgoSuffixFmt.replace("%s", formatDuration(nowMs - latestFetchedAt))}`
		: "";
	...
	for (const { provider, providerReports } of providerEntries) {
		...
		for (const report of providerReports) {
			...
			resetAccountLines.push(
				`    ${M.ccSavedResetsRowFmt
					.replace("%s", label)
					.replace("%s", String(count))
					.replace("%s", count === 1 ? "" : "s")
					.replace("%s", isActive ? M.ccActiveSuffix : "")}`,
			);
				...
				for (const credit of credits) {
					if (credit.expiresAt) {
						...
						if (!Number.isNaN(expiryMs)) {
							...
							if (remaining > 0) {
								resetAccountLines.push(
									`        ${M.ccExpiresInFmt.replace("%s", formatDuration(remaining)).replace("%s", expiryDate)}`,
								);
							} else {
								resetAccountLines.push(`        ${M.ccExpiredFmt.replace("%s", expiryDate)}`);
							}
						}
					}
				}
