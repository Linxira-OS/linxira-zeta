...
export default class Bench extends Command {
	...
	static examples = [
		"# Compare two models across mixed challenges (chat, prefill, generation)\n  zeta bench anthropic/claude-opus-4-5 openai/gpt-5.2",
		"# Fuzzy selectors work\n  zeta bench opus sonnet",
		"# Average over 3 runs each\n  zeta bench opus gpt-5.2 --runs 3",
		"# Isolate prompt-ingestion speed with a 64 KiB cache-busted input\n  zeta bench opus sonnet --profile prefill --prefill-bytes 65536",
		"# Isolate sustained decode throughput\n  zeta bench opus sonnet --profile generation",
		"# Force priority serving tier\n  zeta bench openai-codex/gpt-5.5:low --runs 10 --service-tier priority",
		"# Measure one cold/warm prompt-cache pair\n  zeta bench openai/gpt-5.6 --cache --json",
	];
