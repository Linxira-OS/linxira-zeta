...
import type { VcsCommitAuthor, VcsGitRepo } from "@linxiraos/pi-natives";
import * as natives from "@linxiraos/pi-natives";
import * as vcs from "@linxiraos/pi-natives/vcs";
import { formatBytes, getWorktreeDir, logger, Snowflake } from "@linxiraos/pi-utils";
...
async function writeSyntheticTree(
	...
	const tempIndex = path.join(os.tmpdir(), `zeta-task-index-${Snowflake.next()}`);
...
export async function applyNestedPatches(
	...
	for (const [relativePath, repoPatches] of byRepo) {
		...
		const stashed = (await repository.isDirty())
			? await repository.stashPush(`zeta-isolation-${Snowflake.next()}`)
			: false;
		try {
...
async function replayFilteredAgentCommits(opts: FilteredAgentReplayOptions): Promise<void> {
	...
	const tmpDir = path.join(os.tmpdir(), `zeta-branch-${Snowflake.next()}`);
	try {
