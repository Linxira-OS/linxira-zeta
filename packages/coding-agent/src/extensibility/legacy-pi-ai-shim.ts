 ...
 * `@oh/pi-ai` (or one of its aliased scopes like `@earendil-works/pi-ai`
 ...
 * bare package root. Subpath imports (`@oh/pi-ai/oauth`, etc.)
...
import type { Effort } from "@oh/pi-catalog/effort";
import { clampThinkingLevelForModel, getSupportedEfforts } from "@oh/pi-catalog/model-thinking";
import {
	calculateCost,
	getBundledModel,
	getBundledModels,
 ...
 * `@oh/pi-catalog/models`. `getModel`/`getModels` are the historical
 ...
 * should wrap with `retryTransientCompletion` from `@oh/pi-ai`.
 */
 ...
 * `@oh/pi-ai` barrel no longer forwards. Each symbol still exists in the
 ...
 * `@oh/pi-ai/error`) and the JSON-repair helpers that omp relocated to
 * `@oh/pi-utils`. Upstream root symbols with no omp equivalent are
 * intentionally not shimmed — the package has diverged and there is nothing to
 * forward.
 */
export { isContextOverflow } from "@oh/pi-ai/error";
export { parseJsonWithRepair, parseStreamingJson, repairJson } from "@oh/pi-utils";
