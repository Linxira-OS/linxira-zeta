(function () {
	...
	function bootSession(data) {
			...
			switch (entry.type) {
				case "message": {
					...
					if (msg.role === "assistant") {
						...
						if (textContent) {
							return (
								labelHtml + `<span class="tree-role-assistant">assistant:</span> ${escapeHtml(textContent)}`
							);
						}
				...
				case "custom_message": {
					...
					return (
						labelHtml +
						`<span class="tree-custom">[${escapeHtml(entry.customType)}]:</span> ${escapeHtml(truncate(normalizeTreeText(content)))}`
					);
				}
				case "model_change":
			...
			if (!treeRendered) {
				...
				for (let rowIndex = 0; rowIndex < sidebarRows.length; rowIndex++) {
					...
					if (row.assistantText !== undefined) {
						...
						content.innerHTML =
							labelHtml +
							`<span class="tree-role-assistant">assistant:</span> ${escapeHtml(truncate(normalizeTreeText(row.assistantText)))}`;
					...
					div.addEventListener("click", () =>
						navigateTo(row.navigationId || entry.id, "target", null, row.anchorId),
					);
		...
		function renderToolCall(call, sctx, blockId) {
			...
			return (
				'<omp-tool-view class="tool-execution ' +
				statusClass +
				'" id="' +
				blockId +
				'" data-key="' +
		...
		function renderHeader() {
			const totalCost =
				globalStats.cost.input + globalStats.cost.output + globalStats.cost.cacheRead + globalStats.cost.cacheWrite;
          ...
          <div class="header">
			...
			if (tools && tools.length > 0) {
				...
				const fullHtml = tools
					.map(
						t =>
							`<div class="tool-item"><span class="tool-item-name">${escapeHtml(t.name)}</span> - <span class="tool-item-desc">${escapeHtml(t.description)}</span></div>`,
					)
					.join("");
