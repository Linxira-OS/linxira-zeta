...
describe("legacy-pi specifier shim", () => {
	...
	it("resolves a canonical @oh-my-pi subpath that remaps to itself", async () => {
		// Regression: the resolve hook matches `@linxiraos/pi-*` as well as the
		...
		// `require("@linxiraos/pi-ai/index.js")` first-use boundary — the
		// `/login` provider selector among them.
		const loaded = parseLoaded(await requireThroughShim("@linxiraos/pi-ai/index.js"));
	...
	it("keeps a plugin's canonical subpath import on the host copy, not a plugin-local install", async () => {
		// A plugin that ships its own `@linxiraos/pi-ai` must still share the host
		...
		const localPackage = tempDir.join("node_modules/@linxiraos/pi-ai");
		await Bun.write(
			path.join(localPackage, "package.json"),
			JSON.stringify({ name: "@linxiraos/pi-ai", version: "0.0.0", main: "index.js" }),
		);
		...
		await Bun.write(pluginEntry, 'module.exports = require("@linxiraos/pi-ai/index.js");');
