...
import { isValidJsonSchema, toolWireSchema } from "@linxiraos/pi-ai/utils/schema";
import { validateToolArguments } from "@linxiraos/pi-ai/utils/validation";
import { type TSchema, Type } from "@linxiraos/zeta/extensibility/typebox";
...
/** ArkSchema's `toJsonSchema` returns `unknown`; the shim's wire document is plain JSON Schema. */
function toJsonDocument(schema: TSchema): {
	required?: string[];
	properties?: Record<string, unknown>;
	[key: string]: unknown;
} {
	return schema.toJsonSchema() as {
		required?: string[];
		properties?: Record<string, unknown>;
		[key: string]: unknown;
	};
...
describe("pi.typebox compatibility shim", () => {
	...
	it("composes optional Type.Unsafe schemas without making sibling properties required", () => {
		...
		const document = toJsonDocument(schema);
	...
	it("preserves nested Type.Unsafe keywords fromJsonSchema cannot lower", () => {
		...
		expect(toJsonDocument(Type.Object({ cfg: raw })).properties!.cfg).toEqual(nested);
		expect(toJsonDocument(Type.Object({ cfg: Type.Optional(raw) })).properties!.cfg).toEqual(nested);
	...
	it("applies schema-valued additionalProperties only to undeclared raw-object keys", () => {
		...
		expect(toJsonDocument(schema)).toEqual({
			type: "object",
