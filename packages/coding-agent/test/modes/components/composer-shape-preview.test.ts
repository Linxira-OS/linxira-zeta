...
} from "@linxiraos/zeta/modes/components/composer-shape-preview";
import {
...
describe("composer shape preview", () => {
	...
	it("borrows status rows from the live status source per shape layout", async () => {
		...
		expect(box).toContain("ζ"); // stand-in title forwarded to the status source
