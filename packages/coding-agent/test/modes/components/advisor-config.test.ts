...
describe("advisor config editor warnings and synthetic default row", () => {
	...
	it("surfaces the newly active file's warnings on scope switch, and only there", async () => {
			...
			{ advisors: [{ name: "Reviewer" }] },
			{
				loadDoc: () => {
					pendingLoad = Promise.resolve({
						advisors: [],
						warnings: [
							`${path.join(os.homedir(), ".zeta", "WATCHDOG.yml")}: advisor "\x1b[31mBad\tName\x1b[0m" dropped — boom`,
						],
					});
					return pendingLoad;
				},
		...
		expect(warnings[0]).toContain("~/.zeta/WATCHDOG.yml");
		expect(warnings[0]).not.toContain(path.join(os.homedir(), ".zeta", "WATCHDOG.yml"));
