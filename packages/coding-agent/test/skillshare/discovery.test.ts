...
import { loadSkillshareSkills } from "@linxiraos/zeta/discovery/skillshare";
...
describe("skillshare discovery provider", () => {
	...
	it("loads locked skills from the store and skips entries missing from it", async () => {
		...
		await writeSkillsLock(path.join(project, ".zeta", "skills.lock.json"), projectLock);
	...
	it("keeps skills readable through skill:// when the store sits behind a symlink", async () => {
		...
		await writeSkillsLock(path.join(project, ".zeta", "skills.lock.json"), {
			version: 1,
	...
	it("lets an authored project skill win a name collision", async () => {
		...
		const authored = path.join(project, ".zeta", "skills", "pdf-tools", "SKILL.md");
