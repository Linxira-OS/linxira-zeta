...
// (`~/.zeta/plugins/installed_plugins.json`), the path listClaudePluginRoots
...
async function writeOmpMarketplacePlugin(
	...
	const registryDir = path.join(home, ".zeta", "plugins");
...
describe("discoverAgents", () => {
	...
	test("loads Zeta agents but skips Claude Code custom agents", async () => {
		await fs.mkdir(path.join(projectDir, ".zeta", "agents"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".zeta", "agents", "zeta-test-agent.md"), ZETA_AGENT_MD);
		...
		expect(names).toContain("zeta-test-agent");
