...
describe("profile alias installer", () => {
	it("writes a bash-compatible function that forwards subcommands through zeta", async () => {
		...
		expect(result.command).toBe("zeta --profile=work");
		expect(files.get("/home/me/.bashrc")).toContain("zeta-work() {");
		expect(files.get("/home/me/.bashrc")).toContain('command zeta --profile=work "$@"');
	});
	...
	it("can target the current source invocation instead of the installed zeta binary", async () => {
		const files = new Map<string, string>();
		...
		expect(files.get("/Users/me/.zshrc")).toContain("zeta-work() {");
	...
	it("installs the zsh alias under ZDOTDIR when set", async () => {
		...
		expect(files.get(result.configPath)).toContain("zeta-work() {");
	});
	...
	it("writes a fish function that forwards argv", async () => {
		...
		expect(content).toContain("command zeta --profile=work $argv");
	});
	...
	it("installs the fish alias under XDG_CONFIG_HOME when set", async () => {
		...
		expect(result.configPath).toBe("/home/me/.dotfiles/config/fish/conf.d/zeta-profiles.fish");
		expect(files.get(result.configPath)).toContain("function zeta-work --wraps zeta");
	});
	...
	it("writes a PowerShell function because aliases cannot carry arguments", async () => {
		...
		expect(content).toContain("& zeta --profile=work @args");
	});
	...
	it("detects pwsh from PSModulePath when SHELL is unset on Windows", async () => {
		...
		expect(files.get(result.configPath)).toContain("& zeta --profile=work @args");
	});
	...
	it("replaces a previous block for the same alias", async () => {
		const files = new Map<string, string>([
			[
				...
				[
					"before",
					"# >>> zeta profile alias: zeta-work >>>",
					"alias zeta-work='command zeta --profile=old'",
					"# <<< zeta profile alias: zeta-work <<<",
					"after",
				].join("\n"),
			],
		...
		expect(content).toContain('command zeta --profile=work "$@"');
	...
	it("refuses to rewrite a malformed managed block missing its end marker", async () => {
		...
		const original = ["# >>> zeta profile alias: zeta-work >>>", "zeta-work() {", "export SECRET=keepme"].join("\n");
	...
	it("refuses to shadow the base zeta command case-insensitively", async () => {
		for (const aliasName of ["zeta", "ZETA"]) {
			await expect(
				installProfileAlias({
					profile: "work",
					aliasName,
	...
	it("validates profile names before rendering shell code", async () => {
		...
		).rejects.toThrow("Invalid Zeta profile");
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/zeta-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/zeta-profiles.fish'");
	});
