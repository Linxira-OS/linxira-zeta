...
import { validateJsonSchemaValue } from "@linxiraos/pi-ai/utils/schema/json-schema-validator";
...
import { getConfigRootDir, logger, removeWithRetries, setAgentDir } from "@linxiraos/pi-utils";
...
test("instructions does not split one endpoint into two connections", async () => {
	const configs = await loadFrom(".zeta/mcp.json", {
