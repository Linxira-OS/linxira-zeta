...
import type { ResetCreditAccountStatus, ResetCreditRedeemOutcome, ResetCreditTarget, UsageReport } from "@zeta/pi-ai";
import { Settings } from "@zeta/pi-coding-agent/config/settings";
import type { AgentSession } from "@zeta/pi-coding-agent/session/agent-session";
import type { SessionManager } from "@zeta/pi-coding-agent/session/session-manager";
import { executeAcpBuiltinSlashCommand } from "@zeta/pi-coding-agent/slash-commands/acp-builtins";
import { removeWithRetries, setProjectDir } from "@zeta/pi-utils";
...
describe("wave 5 — adapters and polish", () => {
	...
	it("/mcp add foo --url https://example.com --token X --scope project: outputs success or propagates write error", async () => {
		...
		const mcpModule = await import("@zeta/pi-coding-agent/mcp/config-writer");
	...
	it("/ssh add foo --host x --user y --scope user: calls addSSHHost", async () => {
		const sshModule = await import("@zeta/pi-coding-agent/ssh/config-writer");
	...
	it("/marketplace discover: output is bulleted with '  - ' token", async () => {
		const { MarketplaceManager } = await import("@zeta/pi-coding-agent/extensibility/plugins/marketplace");
