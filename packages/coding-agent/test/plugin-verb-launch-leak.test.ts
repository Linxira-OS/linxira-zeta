 ...
 * `zeta plugin list`). #4845 extended the same footgun to the multi-word
 ...
 * `zeta plugin <action>` command rather than leaking to the model — while
...
describe("documented-but-unregistered plugin verbs do not leak to launch (#2935)", () => {
	test("bare `omp list` hints at `zeta plugin list` instead of launching with 'list' as the prompt", () => {
	...
	test("bare `omp remove` hints at `zeta plugin uninstall` instead of launching with 'remove' as the prompt", () => {
	...
	test("multi-word `omp marketplace add xyz` hints at `zeta plugin marketplace` instead of leaking to the prompt (#4845)", () => {
	...
	test("bare marketplace-family verbs hint at their `zeta plugin` command (#4845)", () => {
