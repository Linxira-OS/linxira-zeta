...
import { QrCode, type QrEcLevel, renderQrHalfBlocks } from "@linxiraos/zeta/utils/qrcode";
...
describe("QR encoder", () => {
	...
	const vectors: ReadonlyArray<{
		...
		{
			text: "https://my.zeta.sh/#mgAYTZwEnpRQtca0CTgn-Q.gdJU",
			ecl: "M",
			mask: 4,
			version: 4,
			size: 33,
	...
	it("deterministically selects a penalty-minimizing mask when none is forced", () => {
		...
		const qr = QrCode.encodeText("https://my.zeta.sh/#demo", "M");
