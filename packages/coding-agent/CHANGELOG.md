...
- 新增 blob 网关端点：外部客户端可上传/拉取会话二进制（图片等）。
- IM 频道转发增加任务深度守卫，嵌套任务不再重复投递 channel sink 消息。
- 新增 Bing 搜索 provider。
- 追踪面板持久化 compaction 摘要，历史压缩后仍可回看。
- 提示缓存仅在内容字节实际变化时刷新，消除无效刷新。
- 新增增量 plan-ultra 工作流；plan 文件按完整默认窗口读取。
