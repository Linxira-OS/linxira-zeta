...
OMP discovers user agents from `~/.zeta/agent/agents/*.md` and project agents from `.zeta/agents/*.md`.
...
Set the role mapping in `~/.zeta/agent/config.yml`:
...
- Project `.zeta` overrides user `.zeta`.
