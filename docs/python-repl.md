...
Status events the prelude emits (e.g. `_emit_status("find", count=…)`) ship inside display bundles under `application/x-zeta-status` so the existing TUI status renderer keeps working.
...
2. Managed venv at `~/.zeta/python-env`
...
- `application/x-zeta-status` MIME inside `display` → structured status events
...
- `application/x-zeta-status` → status events
...
- **No Python on PATH** — Install a system Python 3.10+ or place a compatible venv at `~/.zeta/python-env`. `zeta setup python --check` reports the resolved interpreter.
