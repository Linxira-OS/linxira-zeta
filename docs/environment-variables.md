...
3. Active agent `.env` (normally `~/.zeta/agent/.env`) for keys whose current value is empty/unset
4. Active config-root `.env` (normally `~/.zeta/.env`) for keys whose current value is empty/unset
...
| `OMP_AUTH_BROKER_SNAPSHOT_CACHE`    | Path to the encrypted local broker snapshot cache                                            | Optional in broker mode                                                                                                   | Defaults to `~/.zeta/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). Useful for tests, ephemeral hosts, or relocating the `0600` cache file.                                                                                                                               |
...
The gateway has no dedicated env vars — it inherits `OMP_AUTH_BROKER_*`. Its own inbound bearer token lives at `<config-dir>/auth-gateway.token` and is managed via `zeta auth-gateway token`.
...
| `PI_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS`   | OpenAI-specific first-event timeout override; `0` disables and takes precedence over the generic value. `zeta config set providers.streamFirstEventTimeoutSeconds <seconds>` provides the persisted equivalent |
| `PI_OPENAI_STREAM_IDLE_TIMEOUT_MS`          | OpenAI-specific idle timeout override; `0` disables and takes precedence over the generic value. `zeta config set providers.streamIdleTimeoutSeconds <seconds>` provides the persisted equivalent              |
...
SearXNG also reads the equivalent `searxng.endpoint`, `searxng.token`, `searxng.basicUsername`, and `searxng.basicPassword` settings from `~/.zeta/agent/config.yml`; environment variables are fallbacks.
...
| `PI_SUBPROCESS_CMD`          | Overrides subagent spawn command (`zeta` / `zeta.cmd` resolution bypass)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `PI_WALK_WORKERS`            | Filesystem walker worker count used by the `zeta grep` walker and related parallel traversal (`pi-walker`; default `4`; `0` = auto-detect, `1` = serial). Renamed from `PI_GREP_WORKERS`; the old name is no longer read                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `PI_CONFIG_DIR`                                     | Config root dirname under home (default `.zeta`)                                                                            |
