...
Keep every section factual and independently readable. Tracking stores _project working state_ (plan / progress / blockers / decisions) that travels with the project; it must not store cross-project learned facts — those belong to memory, and cross-references point there by topic.
