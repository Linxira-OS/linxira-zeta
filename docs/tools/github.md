...
- Key collaborators:
  ...
  - `@linxiraos/pi-natives/vcs` — git operations (`vcs.git()` / `vcs.requireGit()`: branch/worktree/config/push).
...
8. `pr_push` reads PR head metadata back from git branch config, derives a refspec, pushes with `repository.push()` (`@linxiraos/pi-natives/vcs`), then invalidates the cached `pr://` rows for the pushed PR via `invalidateAllForNumber()` so the next `pr://` read reflects the push.
...
- Worktree path is `getWorktreeDir("<number>-<repo-hash>")` = `path.join(getWorktreesDir(), "<number>-<repo-hash>")`, where `<number>` is the PR number and `<repo-hash>` is `hashPath(primaryRepoRoot)` (a 7-hex digest of the primary repo root). `getWorktreesDir()` resolves the base in this order: a valid `OMP_WORKTREE_DIR`, the applied `worktree.base` setting, then the profile/XDG-aware data-root default (normally `~/.zeta/wt`). Both overrides expand a leading `~` and must resolve to an absolute path; an invalid relative value is ignored and resolution falls through. `resolveAvailableWorktreePath()` appends a `-2`/`-3`… suffix when the resulting path is already registered with git or present on disk.
- Existing worktree detection is by branch ref `refs/heads/pr-<number>` from `repository.worktrees()` (`@linxiraos/pi-natives/vcs`).
...
- Filesystem
  ...
  - `pr_checkout` may create worktree directories named `<pr-number>-<repo-hash>` under the base selected by `OMP_WORKTREE_DIR`, then `worktree.base`, then the profile/XDG-aware default (normally `~/.zeta/wt`), and add git worktrees there.
...
- Subprocesses / native bindings
  ...
  - `pr_checkout` and `pr_push` also invoke git operations via `@linxiraos/pi-natives/vcs` (`vcs.requireGit()`), serialized by `withRepoLock()` from `packages/coding-agent/src/utils/repo-lock.ts`.
