...
6. A path ending in `:img` selects SVG loading: `splitPathAndSelPreferringLiteral(...)` splits the selector and `loadSvgImageInput(...)` rasterizes `.svg`/`.svgz` bytes to PNG (`rasterizeSvg` from `@linxiraos/pi-natives`, max edge 2048px) before the normal encode/resize pipeline. A `:img` selector on a non-SVG file fails with an explicit error.
