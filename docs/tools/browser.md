...
- **Browser kind**
  - **Headless**: attaches to one project-shared Chromium supervised by the daemon broker (`zeta.browser.headless` / `zeta.browser.headed` in `hub ps`), applies stealth patches, and creates a fresh page per tab. The daemon stops with the last zeta client in the project. Non-CLI hosts launch a private local Chromium instead.
...
- Network
  ...
  - Headless first-use Chromium download uses the in-house `@linxiraos/pi-utils/browsers` installer.
  - Loopback relay mode may start the machine-global `zeta.browser.relay` daemon. The extension connects outbound to the relay, and Puppeteer connects to its CDP-compatible endpoint.
...
- Subprocesses / native bindings
  ...
  - `killExistingByPath()` / `gracefulKillTreeOnce()` use `@linxiraos/pi-natives` process inspection/termination.
...
- Relay mode rejects an unreachable endpoint or a relay whose extension never connects. Loopback CLI-host errors tell the user to run `zeta browser-relay install` and check the extension badge; remote/non-auto-started errors tell the user to start `zeta browser-relay` or check the endpoint.
