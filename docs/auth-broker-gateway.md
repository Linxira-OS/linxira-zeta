...
```
                ...
                │                                                            │
  developer ──▶ │  ┌──────────────────────────┐    ┌────────────────────┐    │
  laptop /      │  │  zeta auth-broker serve   │◀──▶│  SQLite agent.db    │    │
  CI / robomp   │  │  - holds refresh tokens  │    │  (canonical writer)│    │
                ...
                │            ▼                                               │
                │  ┌──────────────────────────┐                              │
                │  │  zeta auth-gateway serve  │  RemoteAuthCredentialStore   │
...
```
zeta auth-broker serve     [--bind=host:port]                    # boot the broker
zeta auth-broker token     [--regenerate] [--json]               # print or rotate the bearer token
zeta auth-broker login     [<provider>] [--via=user@host] [--dry-run]
zeta auth-broker logout    [<provider>]
zeta auth-broker list      [--json]
zeta auth-broker import    <file|dir> [--provider=<id>] [--include-disabled] [--dry-run] [--json]
zeta auth-broker migrate   --from-local [--include-oauth] [--include-env] [--dry-run] [--json]
zeta auth-broker status    [--json]
```
...
```
zeta auth-gateway serve   [--bind=host:port] [--no-auth]
zeta auth-gateway token   [--regenerate] [--json]
zeta auth-gateway status  [--json]
zeta auth-gateway check   [--strict] [--json]
```
