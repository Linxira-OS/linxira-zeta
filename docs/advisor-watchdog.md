...
```sh
zeta -p --advisor "Review this task."
```
...
1. user level: `<active agent dir>/WATCHDOG.md` (`~/.zeta/agent/WATCHDOG.md` by default; relocated by `PI_CODING_AGENT_DIR`)
...
Candidates in hidden owner directories are ignored unless the file is inside an `.zeta` directory. This keeps unrelated dot-directory conventions from being picked up accidentally while still allowing `.zeta/WATCHDOG.md`.
...
- **Usage attribution.** `zeta stats` scans each session folder recursively, so advisor assistant turns (with their usage/cost) are attributed to the same project/session like any other subagent. Advisor "session update" prompts are persisted as `synthetic`, agent-attributed user messages so they never inflate user-message metrics.
