...
- `nativesDir`, normally `~/.zeta/natives`; it uses `$XDG_DATA_HOME/zeta/natives` only when `$XDG_DATA_HOME/zeta` exists;
- `versionedDir`: `<nativesDir>/<packageVersion>`;
- legacy compiled-binary directory: `%LOCALAPPDATA%/zeta` (or `~/AppData/Local/zeta`) on Windows, `~/.local/bin` elsewhere;
...
1. Every selected filename in `@linxiraos/pi-natives-<tag>`.
