...
`/collab` shares your running session with other zeta instances in real time. Guests render the **same session natively in their own TUI** — streaming assistant text, tool-call cards, footer state (cwd, model, context %, cost), ctrl+o expansion, `/dump` — no terminal mirroring. Guests can prompt and interrupt the agent; the host machine runs the agent and all tools.
...
`zeta collab list` (and `/collab list` inside a TUI) enumerates every live Collab host on the local machine under the same zeta configuration root — across terminals, projects, and profiles. Listing is metadata only; it never prints or transmits a link:

```
zeta collab list                          # one row per host, no links
zeta collab list --json                   # {"version": 1, "hosts": [...]}
zeta collab link <instanceId|pid>         # print that host's full-control browser URL
zeta collab link <instanceId|pid> --view  # print its view-only browser URL
zeta collab link <instanceId> --json      # {"version": 1, "instanceId", "generation", "access", "url"}
```
