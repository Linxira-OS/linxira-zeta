...
- **user** (default) -- available in all projects, stored in the user plugins data root's `installed_plugins.json` (`~/.zeta/plugins/installed_plugins.json` by default)
- **project** -- available only in the active project, stored in the nearest project `.zeta/plugins/installed_plugins.json`
...
On Linux and macOS, `zeta config init-xdg` creates the XDG data, state, and cache roots; it does not move existing data. Once the relevant roots exist and `XDG_DATA_HOME`, `XDG_STATE_HOME`, and `XDG_CACHE_HOME` are set, new user marketplace/plugin state resolves under `$XDG_DATA_HOME/zeta` (including `marketplaces.json` and `plugins/`). The `~/.zeta` paths below are the non-XDG defaults.
...
```
zeta plugin marketplace add <source>
zeta plugin marketplace remove <name>
zeta plugin marketplace update [name]
zeta plugin marketplace list
zeta plugin discover [marketplace]
zeta plugin install [--force] [--scope user|project] name@marketplace
zeta plugin uninstall [--scope user|project] name@marketplace
zeta plugin upgrade [--scope user|project] [name@marketplace]
zeta plugin enable [--scope user|project] name@marketplace
zeta plugin disable [--scope user|project] name@marketplace
zeta plugin list
...
Git and local sources must contain a catalog at `.zeta-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). Direct catalog URLs cache only the JSON catalog; plugins in URL-sourced catalogs cannot use relative string sources like `"./plugins/foo"`.

## Catalog format (marketplace.json)

A marketplace catalog lives at `.zeta-plugin/marketplace.json` in the repository root. When zeta is the only intended consumer, prefer this path. To remain Claude Code-compatible (zeta loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — zeta uses it as a fallback when `.zeta-plugin/marketplace.json` is absent. A repository may ship both: zeta reads the `.zeta-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
...
- `zeta plugin upgrade name@marketplace` reinstalls every installed scope when `--scope` is omitted. `/marketplace upgrade name@marketplace`, uninstall, and enable/disable require `--scope user|project` when the plugin exists in both scopes.
...
~/.zeta/
  ...
  plugins/
    ...
    zeta-plugins.lock.json         # Runtime enable/feature state
...
<project>/.zeta/
  plugins/
    ...
    zeta-plugins.lock.json         # Project runtime enable/feature state
    node_modules/<package>        # Symlink to the cached plugin
```
