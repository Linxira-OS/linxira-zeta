...
description: Use when creating a new zeta marketplace. Covers marketplace.json schema, source types, install commands, and publishing.
---

# Authoring Marketplaces

A marketplace is a Git repository (or local directory) that contains a catalog file at either `.zeta-plugin/marketplace.json` (preferred for zeta-specific catalogs) or `.claude-plugin/marketplace.json` (Claude Code-compatible; used as the fallback). Anyone can author one. Users add it with `/marketplace add owner/repo` and then install individual plugins from it.
...
The catalog file lives at either `.zeta-plugin/marketplace.json` or `.claude-plugin/marketplace.json` in the repository root. zeta prefers the `.zeta-plugin/` path and falls back to the Claude path; a repository may publish both to expose tool-specific catalogs from a single source tree.
...
```json
{
  ...
  "metadata": {
    "description": "Official Acme plugins for linxira-zeta"
  },
  "plugins": [
    {
      "name": "acme-linter",
    ...
    },
    {
      ...
      "source": {
        "source": "github",
        "repo": "acme-corp/zeta-deploy-plugin",
        "ref": "main"
      }
    }
  ]
...
my-plugin/
  ...
  package.json                   ← optional version and `zeta.extensions`
...
> Note: extension modules declared via `package.json` `zeta.extensions` **are** loaded from marketplace installs — installation symlinks the cached plugin into the scope's `node_modules` and records it in `zeta-plugins.lock.json`, the same runtime surfaces used by npm-installed and `zeta plugin link`ed plugins.
...
```
zeta plugin marketplace add owner/repo
zeta plugin install name@marketplace-name
```

Scope behavior:

- **user** (default) — installed in the user plugins data root's `installed_plugins.json` (`~/.zeta/plugins/installed_plugins.json` by default), available in all projects. On Linux and macOS, `zeta config init-xdg` creates (but does not migrate data into) the XDG roots; once the relevant roots exist and the XDG variables are set, new user state uses `$XDG_DATA_HOME/zeta/plugins/installed_plugins.json`.
...
1. Create `marketplace.json` at `.zeta-plugin/marketplace.json` (zeta-only) or `.claude-plugin/marketplace.json` (shared with Claude Code) in a new Git repo.
