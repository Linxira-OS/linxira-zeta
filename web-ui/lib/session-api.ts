 ...
 */
export async function deleteProjectSessions(projectRoot: string): Promise<void> {
...
export async function unarchiveSession(id: string): Promise<void> {
	const res = await fetch(`/api/sessions/${encodeURIComponent(id)}/unarchive`, { method: "POST" });
