...
export function buildSessionContext(
	...
	if (leafId === null) return { messages, entryIds, thinkingLevel: "default", model: null };

	const byId = new Map(entries.map(entry => [entry.id, entry]));
	const leaf = leafId !== undefined ? (byId.get(leafId) ?? null) : (entries[entries.length - 1] ?? null);
