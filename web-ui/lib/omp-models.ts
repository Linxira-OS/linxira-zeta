								...
								try {
									...
									if (parsed && typeof parsed === "object" && parsed.response) {
										...
										if (respObj.candidates && Array.isArray(respObj.candidates)) {
											for (const cand of respObj.candidates as Array<{
...
function inferContextWindow(modelId: string): number {
	...
	if (id.includes("deepseek-v4") || id.includes("deepseek-v3.2") || id.includes("meituan") || id.includes("grok"))
		return 1000000;
	if (
		id.includes("kimi-k3") ||
		id.includes("kimi-k2") ||
		id.includes("qwen") ||
		id.includes("doubao") ||
		id.includes("ling-2.6") ||
		id.includes("ring-2.6")
	)
		return 262144;
...
export function syncOmpRuntimeModelsJson(agentDir: string = getAgentDir()): string {
				...
				try {
					...
					if (Array.isArray(rawModels) && rawModels.length > 0) {
						const sanitizedModels = rawModels.map(m => {
							...
							if (
								!cleaned.contextWindow ||
								typeof cleaned.contextWindow !== "number" ||
								(cleaned.contextWindow as number) <= 0
							) {
	...
	if (existsSync(modelsJsonPath)) {
		try {
			const customData = JSON.parse(readFileSync(modelsJsonPath, "utf8")) as {
			...
			if (customData && customData.providers) {
				...
				for (const [providerId, providerConfig] of Object.entries(sanitizedCustom)) {
					...
					const customModelList = Array.isArray(providerConfig.models)
						? (providerConfig.models as Array<Record<string, unknown>>)
						: [];
					...
					for (const customM of customModelList) {
						...
						if (
							...
							typeof merged.contextWindow !== "number" ||
							(merged.contextWindow as number) <= 0
						) {
