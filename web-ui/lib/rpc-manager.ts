...
export class AgentSessionWrapper {
      ...
      if (typeof this.inner.bindExtensions === "function") {
        ...
        await bindExtensions.call(this.inner, {
          ...
          shutdownHandler: () => this.emit({
            ...
            method: "notify",
            notifyType: "warning",
            message: "Extension requested shutdown, but shutdown is not supported in Zeta Web.",
  ...
  private shouldWaitForExtensions(type: string): boolean {
    return type === "prompt" || type === "steer" || type === "follow_up" || type === "get_commands" || type === "set_model" || type === "set_thinking_level";
  }
  ...
  async send(command: Record<string, unknown>): Promise<unknown> {
    ...
    switch (type) {
      ...
      case "get_state": {
        ...
        const rawUsage = this.inner.getContextUsage();
        const effectiveCw = Math.max(rawUsage?.contextWindow ?? 0, (model as { contextWindow?: number })?.contextWindow ?? 0);
        const contextUsage = (rawUsage || effectiveCw > 0)
          ? {
              percent: rawUsage?.percent ?? null,
              contextWindow: effectiveCw > 0 ? effectiveCw : 128000,
      ...
      case "set_model": {
        ...
        const colonIdx = cleanModelId.lastIndexOf(":");
        if (colonIdx > 0) {
          const possibleSuffix = cleanModelId.slice(colonIdx + 1);
          if (THINKING_SUFFIXES.has(possibleSuffix)) {
            thinkingLevel = possibleSuffix;
            cleanModelId = cleanModelId.slice(0, colonIdx);
          }
        }
        ...
        if (thinkingLevel) {
          try {
            await (this.inner as unknown as { setThinkingLevel?: (level: string) => Promise<void> }).setThinkingLevel?.(thinkingLevel);
          } catch {
            // ignore thinking level set error
          }
        }
        invalidateModelsCache();
        invalidateSessionListCache();
        return { id: model?.id ?? cleanModelId, provider: model?.provider ?? cleanProvider };
      }
  ...
  private createExtensionUiContext(): ExtensionUiContextLike {
    return {
      ...
      getAllThemes: () => [],
      getTheme: () => undefined,
      setTheme: () => ({ success: false, error: "Theme switching is not supported in Zeta Web extension UI yet" }),
