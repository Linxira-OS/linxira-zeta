...
const SUB_TABS: { id: SubTab; labelKey: string }[] = [
	{ id: "overview", labelKey: "tracking.tab.overview" },
	{ id: "status", labelKey: "tracking.tab.status" },
	{ id: "plans", labelKey: "tracking.tab.plans" },
	{ id: "logs", labelKey: "tracking.tab.logs" },
	{ id: "sessions", labelKey: "tracking.tab.sessions" },
	{ id: "charts", labelKey: "tracking.tab.charts" },
];
...
export function TrackingPanel({ cwd }: Props) {
	...
	useEffect(() => {
		if (!cwd) return;
		const es = new EventSource(`/api/tracking/events?cwd=${encodeURIComponent(cwd)}`);
		es.onmessage = e => {
			try {
	...
	const renderOverview = () => {
		...
		if (!data.index) {
			return (
				<div style={{ padding: 16, color: "var(--text-dim)", fontSize: 13 }}>{t("tracking.emptyOverview")}</div>
			);
		}
		return (
	...
	const renderStatus = () => {
		if (!data) return null;
		if (!data.status) {
			return <div style={{ padding: 16, color: "var(--text-dim)", fontSize: 13 }}>{t("tracking.emptyStatus")}</div>;
		}
		const s = data.status;
		const listSep = t("tracking.listSeparator");
		const rows: [string, string][] = [
			[t("tracking.field.phase"), s.phase || "-"],
			[t("tracking.field.progress"), s.progress || "-"],
			[t("tracking.field.blockers"), s.blockers.length > 0 ? s.blockers.join(listSep) : "-"],
			[t("tracking.field.decisions"), s.decisions.length > 0 ? s.decisions.join(listSep) : "-"],
			[t("tracking.field.updated"), s.lastUpdated ? new Date(s.lastUpdated).toLocaleString(locale) : "-"],
		];
		return (
	...
	const renderPlans = () => {
		...
		if (data.sessions.length === 0) {
			return <div style={{ padding: 16, color: "var(--text-dim)", fontSize: 13 }}>{t("tracking.emptyPlans")}</div>;
		}
		return (
			<div style={{ padding: "12px 16px", overflow: "auto", flex: 1 }}>
				{data.sessions.map(s => (
	...
	const renderLogs = () => {
		...
		if (data.actions.length === 0) {
			return <div style={{ padding: 16, color: "var(--text-dim)", fontSize: 13 }}>{t("tracking.emptyLogs")}</div>;
		}
	...
	const renderSessions = () => {
		...
		if (data.sessions.length === 0) {
			return (
				<div style={{ padding: 16, color: "var(--text-dim)", fontSize: 13 }}>{t("tracking.emptySessions")}</div>
			);
		}
		return (
			<div style={{ padding: 16, overflow: "auto", flex: 1 }}>
	...
	const renderCharts = () => {
		if (!data) return null;
		if (data.actions.length === 0) {
			return <div style={{ padding: 16, color: "var(--text-dim)", fontSize: 13 }}>{t("tracking.emptyCharts")}</div>;
		}
		...
		const timelineBars = sortedDates.map(([date, count], i) => {
			...
			return (
				<g key={date}>
					<rect x={x} y={y} width={barW} height={barH} rx={2} fill="var(--chart-1)" opacity={0.7} />
					<text
						x={x + barW / 2}
						y={chartH - 6}
						textAnchor="middle"
		...
		return (
			<div style={{ padding: 16, overflow: "auto", flex: 1 }}>
				...
				>
					{[
						{ label: t("tracking.stat.totalOps"), value: data.actions.length },
						{ label: t("tracking.stat.opTypes"), value: opCounts.size },
						{ label: t("tracking.stat.activeDays"), value: dateCounts.size },
						{ label: t("tracking.stat.plans"), value: data.sessions.length },
					].map(s => (
						<div
							key={s.label}
							style={{
	...
	if (error) {
		return (
			...
			>
				<div style={{ fontSize: 13, color: "var(--status-error-foreground)" }}>{t("tracking.loadFailed")}</div>
	...
	return (
		<div style={{ height: "100%", display: "flex", flexDirection: "column", background: "var(--bg)" }}>
			...
			{/* Sub-tab content */}
			<div style={{ flex: 1, overflow: "auto" }}>{subTabContent[activeSubTab]()}</div>
		</div>
	);
}
