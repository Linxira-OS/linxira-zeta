...
function normalizePluginSourceInput(value: string): string {
	const match = value.trim().match(/^\$?\s*zeta\s+install\s+(\S+)\s*$/);
...
function resourceSummary(
	...
	const parts = [
		pkg.counts.extensions
			? t("plugins.resourceCount", {
					count: pkg.counts.extensions,
					label: t("plugins.unit.extensions"),
				})
			: "",
		pkg.counts.skills
			? t("plugins.resourceCount", {
					count: pkg.counts.skills,
					label: t("plugins.unit.skills"),
				})
			: "",
		pkg.counts.prompts
			? t("plugins.resourceCount", {
					count: pkg.counts.prompts,
					label: t("plugins.unit.prompts"),
				})
			: "",
		pkg.counts.themes
			? t("plugins.resourceCount", {
					count: pkg.counts.themes,
					label: t("plugins.unit.themes"),
				})
			: "",
	].filter(Boolean);
...
function versionSummary(
	...
	if (pkg.configuredVersion) parts.push(t("plugins.configuredVersion", { version: pkg.configuredVersion }));
	return parts.length ? parts.join(" · ") : t("plugins.unknown");
}

function installLocation(scope: PluginScope, cwd: string): string {
	return scope === "project" ? `${shortenPath(cwd)}/.zeta/agent/{npm,git}` : "~/.zeta/agent/{npm,git}";
}
...
function ResourceList({ pkg }: { pkg: PluginPackageInfo }) {
	...
	const groups = (
		[
			["extension", t("plugins.group.extensions")],
			["skill", t("plugins.group.skills")],
			["prompt", t("plugins.group.prompts")],
			["theme", t("plugins.group.themes")],
		] as const
	)
		.map(([kind, label]) => ({
			kind,
			...
			{(["global", "project"] as PluginScope[]).map(scope => {
				...
				return (
					<button
						...
						disabled={disabled}
						title={disabled ? t("skills.projectScopeUnavailable") : undefined}
						style={{
							width: 76,
							border: "none",
	...
	return (
		...
		>
			<div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
				...
				>
					<div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)" }}>{t("plugins.addPlugin")}</div>
				</div>
				<div
					style={{
						fontSize: 12,
			...
			>
				<SegmentedScope value={scope} projectResourcesLoaded={projectResourcesLoaded} onChange={onScopeChange} />
				<button
					type="button"
					onClick={onInstall}
				...
				>
					{busy ? t("plugins.installing") : t("plugins.action.install")}
				</button>
			</div>
		...
		>
			...
			>
				...
				<div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
					...
					>
						{busyKey === `update:${key}` ? t("plugins.action.updating") : t("plugins.action.update")}
					</button>
					<button
						onClick={onReloadSession}
						disabled={!sessionId || reloadBusy || busy}
						style={buttonStyle(!sessionId || reloadBusy || busy)}
						title={sessionId ? t("plugins.reloadSession") : t("plugins.openSessionToReload")}
					>
					...
					>
						{busyKey === `remove:${key}` ? t("plugins.action.removing") : t("plugins.action.remove")}
					</button>
				</div>
			</div>
			...
			>
				...
				<div style={{ color: "var(--text-dim)" }}>{t("cwd")}</div>
				<div
					style={{
						color: "var(--text-dim)",
						fontFamily: "var(--font-mono)",
			...
			<div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
				<div style={{ fontSize: 12, fontWeight: 700, color: "var(--text)" }}>{t("plugins.resolvedResources")}</div>
