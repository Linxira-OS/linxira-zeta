...
import { useShikiHighlight } from "@/hooks/useCodeTheme";
...
function ShikiSourceView({ code, language, wrapLines }: { code: string; language: string; wrapLines: boolean }) {
	const { lines } = useShikiHighlight(code, language);
	...
	return (
		<pre
			...
			style={{
				...
				color: "var(--md-syntax-foreground)",
				...FILE_CODE_STYLE,
					...
					>
						...
						>
							{tokens.map((token, tokenIndex) => (
								<span key={`source-token-${tokenIndex}`} style={token.style}>
									{token.text}
								</span>
							))}
						</span>
	...
	return (
		...
		>
			...
			>
				...
				<span title={watching ? t("files.liveSync") : t("files.notWatching")}>
					<span
						style={{
							width: 7,
							height: 7,
			...
			>
				{error ? (
					...
					<img
						...
						onError={() => setError(t("files.loadImageFailed"))}
						style={{
							maxWidth: "100%",
							maxHeight: "100%",
							objectFit: "contain",
	...
	return (
		...
		>
			...
			>
				...
				<span
					title={watching ? t("files.liveSync") : t("files.notWatching")}
					style={{
						display: "flex",
						alignItems: "center",
						gap: 4,
			...
			>
				<div style={{ width: "min(680px, 100%)" }}>
					...
					<audio
						...
						onError={() => setError(t("files.loadAudioFailed"))}
						style={{ width: "100%" }}
					/>
				</div>
			</div>
			...
			<div style={{ flex: 1, minHeight: 0, background: "var(--bg-panel)" }}>
				{error ? (
					...
					<iframe
						...
						title={t("files.previewFile", { file: getFileName(filePath) })}
						style={{
							width: "100%",
							height: "100%",
							border: "none",
...
function TextFileViewer({
	...
	const displayModes: DisplayMode[] = isDeletedDiff
		? ["diff"]
		: ["source", ...(hasPreview ? ["preview" as const] : []), ...(hasGitDiff ? ["diff" as const] : [])];
	const metadata = isDeletedDiff
		? t("files.deleted")
		: `${language} · ${t("files.linesCount", { count: lines.length })} · ${formatSize(data!.size)}`;
		...
		>
			...
			>
				...
				{!isDeletedDiff && (
					<span
						title={watching ? t("files.liveSync") : t("files.notWatching")}
						aria-label={watching ? t("files.liveSync") : t("files.notWatching")}
				...
				<div className="file-viewer-controls">
					{displayModes.length > 1 && (
						<div className="file-viewer-mode-switch" aria-label={t("files.fileViewMode")}>
							{displayModes.map(mode => {
								...
								return (
									<button
										...
										title={mode === "diff" ? t("files.compareHead") : undefined}
										aria-pressed={active}
						...
						{effectiveDisplayMode === "source" && (
							<>
								<button
									...
									title={t("files.mentionSelectedLines")}
									aria-label={t("files.mentionSelectedLines")}
								...
								<button
									...
									title={wrapLines ? t("files.disableWrap") : t("files.enableWrap")}
									aria-label={wrapLines ? t("files.disableWrap") : t("files.enableWrap")}
			...
			>
				{effectiveDisplayMode === "diff" && hasGitDiff ? (
								...
								img({ src, alt, ...props }) {
									...
									const imagePath =
										typeof src === "string"
											...
											: null;
									const imageSrc = imagePath ? getFileApiUrl(imagePath, "read", sourceSessionId) : src;
					...
					<ShikiSourceView code={content} language={language} wrapLines={wrapLines} />
				)}
			</div>
		</div>
	);
