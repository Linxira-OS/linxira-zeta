...
function Toggle({ enabled, loading, onToggle }: { enabled: boolean; loading: boolean; onToggle: () => void }) {
	...
	return (
		<button
			onClick={onToggle}
			disabled={loading}
			title={enabled ? t("skills.visibleInPrompt") : t("skills.hiddenFromPrompt")}
			style={{
				flexShrink: 0,
				width: 40,
				height: 22,
	...
	return (
		<div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
			...
			<div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
				...
				>
					{!enabled && (
						<span style={{ fontSize: 11, color: "var(--text-dim)" }}>{t("skills.hiddenButInvocable")}</span>
					)}
					{saveError && (
						<span
							style={{
				...
				<div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
					...
					>
								...
								{checkingUpdate
									...
									: updateStatus?.state === "up-to-date"
										...
										: updateStatus?.state === "unsupported"
											? t("skills.automaticChecksUnavailable")
						...
						{updateStatus?.state === "update-available" && (
							...
							>
								{updating ? t("skills.updating") : t("skills.update")}
							</button>
						)}
					</div>
...
function AddSkillPanel({
	...
	const installPath = scope === "global" ? "~/.zeta/agent/skills/" : `${shortenPath(cwd)}/.zeta/skills/`;
	...
	return (
		<div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
			...
			>
					...
					>
						{(["global", "project"] as const).map(s => (
							<button
								...
								title={
									s === "project" && !projectResourcesLoaded ? t("skills.projectScopeUnavailable") : undefined
								}
								style={{
									padding: "3px 10px",
									border: "none",
				...
				{installError && (
					<div style={{ fontSize: 12, color: "#f87171", wordBreak: "break-word" }}>{installError}</div>
				)}
			</div>
			...
			{results.length > 0 ? (
				<div style={{ flex: 1, overflowY: "auto" }}>
					{results.map(r => {
						const isInstalled =
							installedPackages[scope].has(r.package) || newlyInstalledPkgs.has(`${scope}:${r.package}`);
												...
												{dormantSkills.length > 0 && (
													<>
														...
														>
															...
															{t("skills.dormant")} ({dormantSkills.length})
														</div>
