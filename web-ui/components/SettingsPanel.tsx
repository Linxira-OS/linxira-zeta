...
function isTerminalEffect(path: string): boolean {
	return path === "language" || path === "symbolPreset" || path.startsWith("theme.") || path.startsWith("statusLine.");
}
	...
	return (
		<input
			...
			style={{
				...
				borderColor: failed ? "#f87171" : undefined,
			}}
		/>
	);
}
...
function ChannelCredentialsForm({
	...
	const hasStoredSecret =
		(isFeishu && (channel.appSecret ?? "") !== "" && channel.appSecret !== "••••") ||
		(isFeishu && channel.appSecret === "••••");
			...
			{isFeishu ? (
				<>
					<input
						...
						placeholder={t("web-channel-app-id")}
					...
					<input
						...
						placeholder={
							hasStoredSecret
								? `${t("web-channel-app-secret")} (${t("settings.secret-kept")})`
								: t("web-channel-app-secret")
						}
						style={{ ...inputStyle, fontFamily: "var(--font-mono)" }}
					/>
					<select
				...
				<input
					...
					placeholder={
						channel.botToken === "••••"
							? `${t("web-channel-bot-token")} (${t("settings.secret-kept")})`
							: t("web-channel-bot-token")
					}
					style={{ ...inputStyle, fontFamily: "var(--font-mono)" }}
				/>
			)}
...
function WebDisplaySection({ t, highlight }: { t: (key: string) => string; highlight?: string }) {
...
function WebSettingsSection({
	...
	const row = (label: string, description: string | undefined, control: React.ReactNode, path?: string) => (
		<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
			<div style={{ display: "flex", flexDirection: "column", gap: 2, minWidth: 0 }}>
				<span style={{ fontSize: 12.5, fontWeight: 600, color: "var(--text)" }}>
					<SettingsHighlight text={label} query={highlight} />
				</span>
				{description && (
					<span style={{ fontSize: 11.5, color: "var(--text-muted)", lineHeight: 1.4 }}>
						<SettingsHighlight text={description} query={highlight} />
					</span>
				)}
			</div>
			<div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
				{path && pending[path] === true && (
...
interface SettingRowProps {
	...
	/** Non-empty query turns on match highlighting for label/description. */
	highlight?: string;
...
function SettingRow({
	...
	const renderControl = () => {
		switch (entry.type) {
			...
			case "providerLimits":
				return (
					<ProviderLimitsEditor value={(value as Record<string, number> | undefined) ?? {}} onCommit={onCommit} />
				);
			case "modelRoles":
				return <ModelRolesEditor value={(value as Record<string, string> | undefined) ?? {}} onCommit={onCommit} />;
			case "multiselect":
				return (
					<MultiSelectEditor entry={entry} value={(value as string[] | undefined) ?? []} onCommit={onCommit} />
				);
		}
	};
	...
	return (
		...
		>
			<div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12 }}>
				<span style={{ fontSize: 12.5, fontWeight: 600, color: "var(--text)" }}>
					<SettingsHighlight text={entry.label} query={highlight} />
				</span>
				{!wide && (
					<div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
						{pending && <span style={{ fontSize: 11, color: "var(--text-dim)" }}>{t("settings.saving")}</span>}
						{renderControl()}
					</div>
				)}
			</div>
			{entry.description && (
				<div style={{ fontSize: 11.5, color: "var(--text-muted)", lineHeight: 1.45 }}>
					<SettingsHighlight text={entry.description} query={highlight} />
				</div>
			)}
			{terminalNote && (
				<div style={{ fontSize: 11, color: "var(--text-dim)" }}>{t("takes-effect-in-the-terminal-cli")}</div>
			)}
			{wide && (
			...
			{error && <div style={{ fontSize: 11.5, color: "#f87171" }}>{t("settings.save-failed-fmt", { error })}</div>}
		</div>
	);
}

/** Highlight every case-insensitive occurrence of `query` inside `text`. */
export function SettingsHighlight({ text, query }: { text: string; query?: string }) {
	...
	while (index !== -1) {
		...
		parts.push(
			<mark
				key={parts.length}
				style={{ background: "var(--accent-muted)", color: "inherit", borderRadius: 2, padding: "0 1px" }}
			>
...
export interface SettingsDataState {
	...
	/** Visible entries of one tab, in gateway order. */
	entriesFor: (tab: string) => SettingEntry[];
	/** Localized group headings for one tab, in render order. */
	groupsFor: (tab: string) => string[];
	commit: (path: string, value: unknown) => Promise<void>;
	reload: () => void;
	/** Render one setting row; a non-empty `highlight` adds match marks. */
	renderRow: (entry: SettingEntry, highlight?: string) => React.ReactNode;
}

export function useSettingsData(enabled = true): SettingsDataState {
	...
	const load = useCallback(async (fetchLang: "en" | "zh") => {
		setLoadError(null);
		try {
			const next = await fetchSettings(fetchLang);
			setData(next);
			setValues(Object.fromEntries(next.settings.map((s): [string, unknown] => [s.path, s.value])));
			setDrafts({});
			setErrors({});
			setActiveTab(cur =>
				cur === "web" || cur === "docs" || next.tabs.some(tab => tab.id === cur)
					? cur
					: (next.tabs[0]?.id ?? "appearance"),
			);
		} catch (err) {
	...
	const renderRow = (entry: SettingEntry, highlight?: string) => {
		if (!EDITABLE_TABS.has(activeTab)) {
			return (
				...
				>
					...
					<div style={{ fontSize: 11, color: "var(--text-dim)" }}>{t("settings.configure-via-cli")}</div>
				</div>
			);
		}
		return (
	...
	const entriesFor = (tab: string) => (data ? data.settings.filter(s => s.tab === tab && s.visible) : []);
...
export interface SettingsWebConfigState {
	...
	pending: Record<string, boolean>;
	savedFlash: string | null;
	commit: (path: string, value: unknown) => Promise<boolean>;
	reload: () => Promise<void>;
}

export function useWebConfigState(enabled = true): SettingsWebConfigState {
	const { t } = useI18n();
	const [data, setData] = useState<WebConfigData | null>(null);
	const [error, setError] = useState<string | null>(null);
	const [pending, setPending] = useState<Record<string, boolean>>({});
	...
	const commit = useCallback(
		async (path: string, value: unknown): Promise<boolean> => {
			...
			try {
				...
				setData(cur => (cur ? setAtPath(cur, path, value) : cur));
				...
				setSavedFlash(t("web-config-saved"));
				return true;
			} catch (err) {
				setError(err instanceof Error ? err.message : String(err));
				return false;
	...
	return { data, error, pending, savedFlash, commit, reload };
}

function WebTabContent({ web, highlight }: { web: SettingsWebConfigState; highlight?: string }) {
	const { t } = useI18n();
	if (web.data) {
		return (
			<>
				<WebDisplaySection t={t} highlight={highlight} />
				<WebSettingsSection
					data={web.data}
					pending={web.pending}
					error={web.error}
					savedFlash={web.savedFlash}
					t={t}
					onCommit={web.commit}
					highlight={highlight}
				/>
		...
		<div style={{ padding: 24, fontSize: 12.5, color: "var(--text-muted)" }}>
			{web.error ? (
				<div style={{ display: "flex", flexDirection: "column", gap: 10, alignItems: "flex-start" }}>
					...
					<button type="button" onClick={() => void web.reload()} style={retryButtonStyle}>
						{t("refresh")}
					</button>
				</div>
			) : (
...
export interface SettingsTabBodyProps {
	activeTab: string;
	data: SettingsResponse | null;
	loadError: string | null;
	reload: () => void;
	web: SettingsWebConfigState;
	renderRow: (entry: SettingEntry, highlight?: string) => React.ReactNode;
	onOpenModelsConfig: () => void;
	/** Non-empty query filters rows and highlights matching labels. */
	query?: string;
}
...
export function SettingsTabBody({
	...
	if (activeTab === "docs") {
		body = <DocsPanel locale={locale} t={t} />;
	} else if (activeTab === "web") {
		body = <WebTabContent web={web} highlight={q} />;
	} else if (loadError) {
		body = (
			<div
				style={{
			...
			>
				<span>{t("settings.load-failed-fmt", { error: loadError })}</span>
				<button type="button" onClick={reload} style={retryButtonStyle}>
					{t("settings.retry")}
				</button>
			</div>
		);
	} else if (!data) {
		body = <div style={{ padding: 24, fontSize: 12.5, color: "var(--text-muted)" }}>{t("loading")}</div>;
	} else {
		const entries = data.settings.filter(s => s.tab === activeTab && s.visible);
		const filtered =
			q === ""
				? entries
				: entries.filter(
						s => s.label.toLowerCase().includes(q) || (s.description?.toLowerCase().includes(q) ?? false),
					);
		const groupOrder = data.groups[activeTab] ?? [];
		const ungrouped = filtered.filter(s => s.group === undefined);
		const grouped = groupOrder
			.map(group => ({ group, items: filtered.filter(s => s.group === group) }))
		...
		body = (
			<>
				...
				{ungrouped.map(entry => renderRow(entry, q))}
				{grouped.map(({ group, items }) => (
					<div key={group}>
						...
						>
							<SettingsHighlight text={group} query={q} />
						</div>
						{items.map(entry => renderRow(entry, q))}
					</div>
				))}
				{entries.length === 0 && (
					<div style={{ padding: 24, fontSize: 12.5, color: "var(--text-muted)" }}>{t("settings.tab-empty")}</div>
				)}
				{entries.length > 0 && filtered.length === 0 && (
					<div style={{ padding: 24, fontSize: 12.5, color: "var(--text-muted)" }}>
						{t("settings.search-no-results", { query: query ?? "" })}
					</div>
				)}
			</>
		);
