...
import type { DiscoveredModel, ModelCatalogPreset, ModelCatalogRecommendation } from "@/lib/api-types";
...
function Select({
	...
	return (
		...
		>
			{!required && <option value="">— {t("models.defaultOption")} / none —</option>}
			{options.map(o => (
...
function ProviderDetail({
	...
	const allShownSelected =
		selectableShownIds.length > 0 && selectableShownIds.every(id => selectedModelIds.includes(id));
	const someShownSelected = !allShownSelected && selectableShownIds.some(id => selectedModelIds.includes(id));
	...
	const toggleDiscoveredModel = (id: string) => {
		setSelectedModelIds(current => (current.includes(id) ? current.filter(entry => entry !== id) : [...current, id]));
	};
		...
		<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
			...
			<Field label="API Key">
				...
				<span style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>
					Prefix with <code style={{ fontFamily: "var(--font-mono)" }}>!</code> to run a shell command, or use an
					env var name
				</span>
			</Field>
						...
						style={{
							...
							color:
								!provider.baseUrl?.trim() || discoveryState.phase === "loading"
									...
									: "var(--text-muted)",
							cursor:
								!provider.baseUrl?.trim() || discoveryState.phase === "loading" ? "not-allowed" : "pointer",
							fontSize: 11,
						}}
					>
						...
						>
							...
							<button
								...
								style={{
									...
									border: "none",
									borderRadius: 5,
									background: selectedCount ? "var(--accent)" : "var(--bg-panel)",
...
function ModelDetail({
	...
	const lookupMetadata = useCallback(async () => {
		...
		try {
			...
			if (!res.ok || !data.ok || (data.contextWindow === undefined && data.maxTokens === undefined)) {
				...
				setMetadataMessage(data.error ?? t("models.metadataNone"));
				return;
			}
			onChange({
				...model,
				...(data.contextWindow !== undefined && model.contextWindow === undefined
					? { contextWindow: data.contextWindow }
					: {}),
				...(data.maxTokens !== undefined && model.maxTokens === undefined ? { maxTokens: data.maxTokens } : {}),
			});
			setMetadataState("success");
			setMetadataMessage(data.source ?? t("models.metadataSourceFound"));
		} catch (error) {
			setMetadataState("error");
			setMetadataMessage(error instanceof Error ? error.message : String(error));
		}
	}, [metadataState, model, onChange, provider, providerName, t]);
	...
	const catalogResultSummary = (() => {
		...
		const applied =
			appliedCount > 0 ? t("models.catalogFilled", { count: appliedCount }) : t("models.catalogNoEmptyFields");
		...
		const price =
			recommendation.price.method === "provider"
				? t("models.catalogPriceProvider", {
						provider: recommendation.price.providerName ?? recommendation.price.providerId ?? providerName,
					})
	...
	const catalogStatusText = catalogState.phase === "error" ? catalogState.message : catalogResultSummary;
		...
		<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
			...
			>
				...
				<div style={{ display: "flex", alignItems: "center", gap: 8 }}>
					...
					<button
						...
						title={t("models.testConnectionTitle")}
						style={{
							height: 24,
							padding: "0 8px",
				...
				>
					<button
						...
						style={{
							...
							background: "var(--bg-panel)",
							color:
								!model.id.trim() || catalogState.phase === "loading" ? "var(--text-dim)" : "var(--text-muted)",
			...
			>
				...
				<button
					...
					title={metadataMessage || t("models.metadataLookupTitle")}
					style={{
						height: 28,
						padding: "0 9px",
						background: metadataState === "success" ? "#16a34a" : "none",
						border: `1px solid ${metadataState === "error" ? "#ef4444" : metadataState === "success" ? "#16a34a" : "var(--border)"}`,
						borderRadius: 4,
						color:
							metadataState === "success" ? "#fff" : metadataState === "error" ? "#ef4444" : "var(--text-muted)",
						cursor: !model.id.trim() || metadataState === "loading" ? "not-allowed" : "pointer",
						fontSize: 11,
						whiteSpace: "nowrap",
					}}
				>
					{metadataState === "loading"
						? t("models.metadataLookingUp")
						: metadataState === "success"
							? t("models.metadataFound")
							: t("models.metadataAutoFill")}
				</button>
			</div>
			{metadataMessage && metadataState !== "success" && (
				<div
					style={{
						marginTop: -8,
						fontSize: 10,
						color: metadataState === "error" ? "#ef4444" : "var(--text-dim)",
					}}
				>
					{metadataMessage}
				</div>
			...
			{model.reasoning && (
				<>
					...
					<div>
						...
						<ThinkingConfigEditor value={model.thinking} onChange={v => set("thinking", v)} />
					</div>
					<div>
						<div
							style={{
			...
			<div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
				<Check
					...
					onChange={v => set("supportsTools", v || undefined)}
				/>
				<Check
					label="Omit max output tokens"
				...
				<div style={{ display: "flex", alignItems: "center", gap: 6 }}>
					...
					<NumInput
						...
						onChange={v => set("premiumMultiplier", v ? parseFloat(v) : undefined)}
						placeholder="1"
					/>
				</div>
			</div>
		...
		<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
			...
			>
				...
				<div style={{ display: "flex", alignItems: "center", gap: 6 }}>
					...
					>
						{provider.loggedIn ? t("models.connectedStatus") : t("models.notConnectedStatus")}
					</span>
				</div>
			</div>
			...
			<div style={{ minHeight: 48 }}>
				...
				{loginState.phase === "success" && (
					<p style={{ margin: 0, fontSize: 12, color: "#4ade80" }}>{t("models.connectedSuccessfully")}</p>
				)}
...
export function ModelsConfig({ onClose }: { onClose: () => void }) {
	...
	}, [reloadConfig, refreshAuthProviders]);
						...
						>
							...
							{importMessage && (
								<span
									style={{
										fontSize: 11,
										color: importMessage.startsWith("Imported") ? "var(--text-muted)" : "#f87171",
										maxWidth: 200,
