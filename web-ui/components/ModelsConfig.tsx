...
function Select({
  ...
  return (
    ...
    >
      {!required && (
        <option value="">— {t("models.defaultOption")} / none —</option>
      )}
      {options.map((o) => (
      ...
      <Field label="Default level">
        <Select
          ...
          options={THINKING_LEVELS.filter(
            (l) => value?.efforts?.includes(l) ?? true,
          )}
        />
      </Field>
    </div>
...
function CompatEditor({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
      ...
      />
      {error && (
        <span style={{ fontSize: 10.5, color: "#f87171" }}>{error}</span>
      )}
    </div>
  );
}
...
function ModelDetail({
  ...
  const lookupMetadata = useCallback(async () => {
    ...
    try {
      ...
      if (
        !res.ok ||
        !data.ok ||
        (data.contextWindow === undefined && data.maxTokens === undefined)
      ) {
      ...
      onChange({
        ...
        model.contextWindow === undefined
          ...
          : {}),
        ...(data.maxTokens !== undefined && model.maxTokens === undefined
  ...
  const testSummary = (() => {
    ...
    return [t("models.failed"), ...meta, testState.message]
      .filter(Boolean)
      .join(" · ");
  })();
            ...
            color:
              metadataState === "success"
                ...
                : metadataState === "error"
                  ...
                  : "var(--text-muted)",
            cursor:
              !model.id.trim() || metadataState === "loading"
                ? "not-allowed"
                : "pointer",
            fontSize: 11,
            whiteSpace: "nowrap",
          ...
          <NumInput
            value={
              model.premiumMultiplier !== undefined
                ? String(model.premiumMultiplier)
                : ""
            }
            onChange={(v) =>
              set("premiumMultiplier", v ? parseFloat(v) : undefined)
            }
            placeholder="1"
          />
        </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          >
            {provider.loggedIn
              ? t("models.connectedStatus")
              : t("models.notConnectedStatus")}
          </span>
        </div>
      </div>
...
export function ModelsConfig({ onClose }: { onClose: () => void }) {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/models/import?base=${encodeURIComponent(url)}`,
      );
      ...
      if (!response.ok) {
        ...
        setImportMessage(
          `Imported ${data.imported ?? 0} models (${data.skipped ?? 0} skipped).`,
        );
        reloadConfig();
      }
    } catch {
  ...
  const activeOAuth = oauthProviders.filter(
    (p) => p.loggedIn && !customIds.has(p.id),
  );
  const activeApiKey = apiKeyProviders.filter(
    (p) => p.configured && !customIds.has(p.id),
  );
