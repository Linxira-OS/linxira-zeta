...
function AppShellContent() {
  ...
  const [desktopOpenTargets, setDesktopOpenTargets] = useState<
    DesktopOpenTarget[]
  >([]);
  const [updating, setUpdating] = useState(false);
  const [checkingUpdate, setCheckingUpdate] = useState(false);
  const [updateNotice, setUpdateNotice] = useState<string | null>(null);
  const updateNoticeTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
      ...
      try {
        ...
        if (hasDesktopOpenBridge() && openOptions?.desktop) {
          ...
          await openDesktopTarget(
            target,
            (await response.json()) as GatewayOpenPath,
          );
          return;
        }
  ...
  const handleRunUpdate = useCallback(async () => {
    ...
    try {
      ...
      window.alert(
        `Updated to ${updateInfo.latest}. Restart Zeta to use the new version.`,
      );
      setUpdateInfo(null);
    } finally {
      setUpdating(false);
  ...
  useEffect(() => {
    ...
    else if (!sessionPanelVisible && was)
      setDockTool((cur) => (cur === "session" ? null : cur));
  }, [sessionPanelVisible]);
  ...
  const handleCloseFileTab = useCallback(
    (tabId: string) => {
      setFileTabs((prev) => {
        ...
        if (next.length === 0)
          setDockTool((cur) => (cur === "files" ? null : cur));
        return next;
      });
      ...
      {desktopChrome && (
        ...
        >
          ...
          >
            ...
            <TitlebarButton
              label={
                windowMaximized ? t("titlebar.restore") : t("titlebar.maximize")
              }
        ...
        >
          ...
          >
            <button
              ...
              title={
                sidebarOpen ? t("layout.hideSidebar") : t("layout.showSidebar")
              }
              aria-label={
                sidebarOpen ? t("layout.hideSidebar") : t("layout.showSidebar")
              }
                  ...
                  onClick={() => {
                    ...
                    if (updateInfo) {
                      if (
                        ...
                        window.confirm(`Update Zeta to ${updateInfo.latest}?`)
                      ) {
                  ...
                  >
                    ...
                    {desktopOpenTargets.length === 0 &&
                      openOptions.terminal && (
                        <button
                          onClick={() => void handleOpenTarget("terminal")}
                          style={openMenuItemStyle}
                        >
                          Terminal
                        </button>
                    ...
                    {desktopOpenTargets.length === 0 &&
                      openOptions.explorer && (
                        <button
                          onClick={() => void handleOpenTarget("explorer")}
                          style={openMenuItemStyle}
                        >
                          Explorer
                        </button>
                    ...
                    {desktopOpenTargets.length === 0 &&
                      openOptions.editors.map((editor) => (
                        <button
                          key={editor}
                          onClick={() =>
                            void handleOpenTarget("editor", editor)
                          }
                          style={openMenuItemStyle}
                        >
                          {editor}
            ...
            {showChat &&
              ...
              (() => {
                ...
                if (contextUsage?.contextWindow) {
                  ...
                  if (pct !== null && pct > 90)
                    ctxColor = "var(--status-error-foreground)";
                  else if (pct !== null && pct > 70)
                    ctxColor = "var(--status-warning-foreground)";
                  ctxStr =
                    pct !== null
                ...
                if (t) {
                  ...
                  tooltipParts.push(
                    `cache read: ${t.cacheRead.toLocaleString(locale)}`,
                  );
                  tooltipParts.push(
                    `cache write: ${t.cacheWrite.toLocaleString(locale)}`,
                  );
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                color:
                  dockTool === "files" ? "var(--accent)" : "var(--text-muted)",
                cursor: "pointer",
          ...
          {(
            ...
            return (
              <button
                key={entry.id}
                onClick={() =>
                  setDockTool((cur) => (cur === entry.id ? null : entry.id))
                }
                title={entry.label}
