						...
						>
							{canNavigate && (
								<button
									...
									title={t("message.edit-from-here-title")}
									style={{
										display: "flex",
										alignItems: "center",
										gap: 4,
							...
							{canFork && (
								<button
									...
									disabled={forking}
									title={forking ? t("message.creating-session-title") : t("message.new-session-title")}
									style={{
										display: "flex",
										alignItems: "center",
										gap: 4,
	...
	return (
		<div style={{ marginBottom: 16 }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
							...
							<>
								{est > 0 && (
									<span
										...
										title={t("message.estimated-tokens")}
									>
										<span
											style={{
												display: "flex",
										...
										{tps !== null &&
											(() => {
												const status =
													tps >= 50 ? "info" : tps >= 30 ? "success" : tps >= 15 ? "warning" : "error";
												return (
													<span
														data-status={status}
														style={{
			...
			{providerError && (
				...
				>
					{t("message.provider-error-fmt", { error: providerError })}
				</div>
			)}
					...
					<details style={{ position: "relative" }}>
						...
						<div
							style={{
								...
								borderRadius: 8,
								boxShadow: "0 8px 24px var(--surface-shadow)",
								padding: "4px 0",
								zIndex: 60,
							}}
						>
	...
	return (
		...
		>
			...
			>
				<span
					style={{
						color: isError ? "var(--status-error-foreground)" : "var(--status-success-foreground)",
						display: "flex",
						alignItems: "center",
						gap: 5,
						flexShrink: 0,
			...
			{files.map((file, fileIndex) => (
				...
				>
					{showFileHeaders && (
						...
						>
							<SplitDiffHeader title={file.oldPath || t("message.diff-before")} side="left" />
							<SplitDiffHeader title={file.newPath || t("message.diff-after")} side="right" />
						</div>
					)}
...
function PairedResult({ text, isEmpty, isError }: { text: string; isEmpty: boolean; isError: boolean }) {
	...
	return (
		...
		>
			...
			>
				{isEmpty ? t("message.no-output") : text}
			</pre>
		</div>
	);
}
		...
		<div style={{ marginBottom: 16 }}>
			...
			>
				...
				<div style={{ padding: "11px 13px 12px" }}>
					...
					>
						{t("message.conversation-compacted")}
					</div>
					<div
						style={{
							marginTop: 3,
					...
					>
						{t("message.compaction-description")}
					</div>
					{parsedSummary.body ? (
						<MarkdownBody className="markdown-compaction-message">{parsedSummary.body}</MarkdownBody>
					) : (
						<span style={{ color: "var(--text-dim)", fontSize: 12 }}>{t("message.no-summary")}</span>
					)}
...
function CompactionFileMetadata({ readFiles, modifiedFiles }: { readFiles: string[]; modifiedFiles: string[] }) {
	...
	return (
		<details className="compaction-file-details">
			<summary>{t("message.file-context-fmt", { details: parts.join(", ") })}</summary>
			{modifiedFiles.length > 0 && <CompactionFileList title={t("message.modified-files")} files={modifiedFiles} />}
			{readFiles.length > 0 && <CompactionFileList title={t("message.read-files")} files={readFiles} />}
		</details>
	);
}
		...
		<div style={{ marginBottom: 16 }}>
			...
			>
				...
				>
					...
					{isHiddenDisplay && (
						<span style={{ color: "var(--text-dim)", fontSize: 11 }}>{t("message.hidden-extension")}</span>
					)}
					{time && (
						<span
							style={{
				...
				{contentExpanded ? (
					<div style={{ padding: "6px 9px" }}>
						...
						{text ? (
							...
							<span style={{ color: "var(--text-dim)", fontSize: 12 }}>{t("message.no-message")}</span>
						)}
					</div>
				) : (
					<button
					...
					>
						{text ? previewText(text, t("message.show-extension")) : t("message.show-extension")}
					</button>
				)}
						...
						>
							{isHiddenDisplay
								? contentExpanded
									...
									: t("message.expand")
								: detailsExpanded
									? t("message.hide-details")
									: t("message.show-details")}
						</button>
					)}
				</div>
...
function previewText(text: string, emptyFallback: string): string {
		...
		<div style={{ margin: "6px 0" }}>
			...
			{message.truncated && fullOutputUrl && (
				<div style={{ padding: "4px 10px", fontSize: 11, marginTop: -1 }}>
					...
					{fullError && (
						<span style={{ marginLeft: 6, color: "var(--text-dim)", fontSize: 11 }}>({fullError})</span>
					)}
				</div>
			)}
		</div>
