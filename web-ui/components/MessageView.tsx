            ...
            >
              ...
              {canFork && (
                <button
                  ...
                  title={
                    forking
                      ? t("message.creating-session-title")
                      : t("message.new-session-title")
                  }
                  style={{
                    display: "flex",
                    alignItems: "center",
  ...
  const touchedFiles: string[] = (() => {
    ...
    for (const b of blocks) {
      ...
      for (const [key, value] of Object.entries(input)) {
        if (
          typeof value === "string" &&
          /path|file|file_path|note_path/i.test(key) &&
          /[\\/]/.test(value)
        ) {
          found.add(value);
        }
    ...
    <div
      ...
      style={{
        ...
        border: isError
          ...
          : "1px solid var(--chat-tool-border)",
        background: isError
          ? "var(--status-error-background)"
          : "var(--chat-tool-background)",
      }}
    >
      {/* ── Tool call header ── */}
      <button
      ...
      {files.map((file, fileIndex) => (
        ...
        >
          {showFileHeaders && (
            ...
            >
              <SplitDiffHeader
                title={file.oldPath || t("message.diff-before")}
                side="left"
              />
              <SplitDiffHeader
                title={file.newPath || t("message.diff-after")}
                side="right"
              />
            </div>
          )}
...
function CompactionFileMetadata({
  ...
  return (
    <details className="compaction-file-details">
      <summary>
        {t("message.file-context-fmt", { details: parts.join(", ") })}
      </summary>
      {modifiedFiles.length > 0 && (
        <CompactionFileList
          title={t("message.modified-files")}
          files={modifiedFiles}
        />
      )}
      {readFiles.length > 0 && (
        <CompactionFileList title={t("message.read-files")} files={readFiles} />
      )}
    </details>
  );
}
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      >
        ...
        {contentExpanded ? (
          ...
          >
            {text
              ? previewText(text, t("message.show-extension"))
              : t("message.show-extension")}
          </button>
        )}
