...
export function FolderPickerModal({ open, initialPath, onSelect, onClose }: Props) {
	...
	const filteredDirs = directories.filter(d => d.name.toLowerCase().includes(filterQuery.trim().toLowerCase()));
		...
		>
			...
			>
					...
					{drives.map(drive => (
						<button
							...
							style={{
								...
								border: "1px solid var(--border)",
								borderRadius: 4,
								color: currentPath.toUpperCase().startsWith(drive.toUpperCase()) ? "#fff" : "var(--text)",
								cursor: "pointer",
								fontFamily: "var(--font-mono)",
								fontWeight: 600,
							}}
				...
				<div style={{ flex: 1, overflowY: "auto", padding: "6px 8px" }}>
					...
					{error && (
						<div style={{ padding: 16, color: "#ef4444", fontSize: 12, textAlign: "center" }}>{error}</div>
					)}
				...
				>
					<div style={{ flex: 1, minWidth: 0 }}>
						<div
							style={{ fontSize: 10, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: 0.5 }}
						>
							{t("selected-path")}
						</div>
						<div
