...
export const BUILTIN_SLASH_COMMANDS: SlashCommandPaletteItem[] = [
  ...PI_CLI_BUILTIN_SLASH_COMMANDS.map(
...
function getSlashDescription(
  ...
  return command.description.startsWith("chat.") ||
    command.description.startsWith("slashcmd.")
...
function buildSlashCommandLayout(
  ...
  for (const command of commands) {
    (isDormantSkillCommand(command, skillDormancy) ? dormant : active).push(
      command,
    );
  }
...
export function ModelErrorBanner({ error }: { error?: string | null }) {
  ...
  return (
    <ModelNoticeBanner tone="error" title={t("model-error")} body={error} />
  );
}
...
export function ModelScopeWarningBanner({ warnings }: { warnings?: string[] }) {
  ...
  return (
    <ModelNoticeBanner
      ...
      title={
        warnings.length > 1
          ? t("chat.model-scope-warnings")
          : t("chat.model-scope-warning")
      }
      body={warnings.join("\n")}
    />
  );
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput(
  ...
  const modelActiveFlashTimerRef = useRef<
  ...
  const flatModelOptions: ModelOption[] = modelsByProvider.flatMap(
    (g) => g.options,
  );
    ...
    >
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
            ...
            >
              ...
              >
                ...
                <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <Kbd>↑↓</Kbd>
                  <span style={{ color: "var(--text-dim)" }}>选择</span>
                  <Kbd>Tab</Kbd>
                  <span style={{ color: "var(--text-dim)" }}>补全</span>
                  <Kbd>Esc</Kbd>
                  <span style={{ color: "var(--text-dim)" }}>关闭</span>
                </span>
              </div>
              <div
                style={{
        ...
        {bashMode && (
          ...
          >
            {t("chat.shell")} ·{" "}
            {bashExcluded ? t("chat.output-local") : t("chat.output-model")}
          </div>
        )}
                                ...
                                <div key={group.provider}>
                                  ...
                                  {group.options.map((opt) => {
                                    const isActive =
                                      ...
                                      opt.provider === model?.provider;
                                    const flatIdx =
                                      flatModelOptions.indexOf(opt);
                                    const isKbdActive =
                                      flatIdx === modelActiveIndex &&
                                      modelDropdownOpen;
                                    return (
                                      <button
                                        key={`${opt.provider}:${opt.modelId}`}
                                        onMouseEnter={() =>
                                          setModelActiveIndex(flatIdx)
                                        }
                                        onClick={() => {
                                          setModelDropdownOpen(false);
                                          setModelFilter("");
