...
export function filterModelOptions(options: ModelOption[], query: string): ModelOption[] {
	...
	return options.filter(
		opt =>
			...
			opt.modelId.toLowerCase().includes(normalized) ||
			formatProviderName(opt.provider).toLowerCase().includes(normalized) ||
			opt.provider.toLowerCase().includes(normalized),
	);
}

const THINKING_LEVELS = ["auto", "off", "minimal", "low", "medium", "high", "xhigh", "max"] as const;
const THINKING_LEVEL_DESC_KEYS: Record<(typeof THINKING_LEVELS)[number], string> = {
	auto: "chat.thinking-use-default",
...
const WEB_COMMAND_ALIASES: SlashCommandPaletteItem[] = [
	...
	{ name: "models", description: "chat.commandModels", source: "builtin" },
	{ name: "clear", description: "chat.commandClear", source: "builtin" },
	{ name: "help", description: "chat.commandHelp", source: "builtin" },
...
function isDormantSkillCommand(command: SlashCommandPaletteItem, skillDormancy: Record<string, boolean>): boolean {
	return command.source === "skill" && skillDormancy[command.name] === true;
}
...
function buildSlashCommandLayout(
	...
	const active: SlashCommandPaletteItem[] = [];
	const dormant: SlashCommandPaletteItem[] = [];
	for (const command of commands) {
		(isDormantSkillCommand(command, skillDormancy) ? dormant : active).push(command);
	}
	const ordered = [...active, ...dormant];
	const groups: SlashCommandGroup[] = [];
	ordered.forEach((command, index) => {
		let group = groups.find(g => g.source === command.source);
		if (!group) {
			group = { source: command.source, items: [] };
			groups.push(group);
		}
		group.items.push({ command, index });
	});
	return { commands: ordered, groups };
}
...
function draftImagesToAttachedImages(images: ChatDraftImage[] | undefined): AttachedImage[] {
	return (images ?? []).filter(isBase64ImageWithinLimits).slice(0, MAX_ATTACHED_IMAGES).map(draftImageToAttachedImage);
}
...
export function ModelErrorBanner({ error }: { error?: string | null }) {
	...
	return <ModelNoticeBanner tone="error" title={t("model-error")} body={error} />;
}
...
export function ModelScopeWarningBanner({ warnings }: { warnings?: string[] }) {
	...
	return (
		<ModelNoticeBanner
			tone="warning"
			title={warnings.length > 1 ? t("chat.model-scope-warnings") : t("chat.model-scope-warning")}
			body={warnings.join("\n")}
		/>
	);
}
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput(
	...
	const skillDormancy = cwd && skillDormancyState?.cwd === cwd ? skillDormancyState.values : {};
			...
			if (modelDropdownOpen) {
				...
				if ((e.key === "Tab" || (e.key === "Enter" && !e.shiftKey)) && flat[modelActiveIndex]) {
					...
					if (opt.modelId !== model?.modelId || opt.provider !== model?.provider || isAutoModelSelection) {
						onModelChange?.({ provider: opt.provider, modelId: opt.modelId });
					}
					return;
				}
			}
	...
	const compactResultText = compactResult
		? `${compactResult.reason && compactResult.reason !== "manual" ? `${compactResult.reason[0].toUpperCase()}${compactResult.reason.slice(1)} ` : t("chat.compacted")} ${formatTokenCount(compactResult.tokensBefore)} -> ${formatTokenCount(compactResult.estimatedTokensAfter)} tokens (${t("chat.tokens-saved", { saved: formatTokenCount(compactSavedTokens) })})`
		: null;
		...
		>
			...
			<div style={{ maxWidth: 820, margin: "0 auto" }}>
				...
				{(queuedMessages?.steering.length ?? 0) + (queuedMessages?.followUp.length ?? 0) > 0 && (
					...
					>
						...
						>
							...
							>
								{t("chat.queued", {
									count: (queuedMessages?.steering.length ?? 0) + (queuedMessages?.followUp.length ?? 0),
								})}
							</span>
							{onRecallQueue && (
								<button
					...
					{slashMenuOpen && slashQuery !== null && (
						...
						>
							...
							>
								<span>
									{slashCommandsLoading
										? t("chat.loading-commands")
										: t("chat.slash-commands-fmt", {
												label: slashCommandCountLabel,
											})}
								</span>
								<span style={{ display: "flex", alignItems: "center", gap: 6 }}>
									<Kbd>↑↓</Kbd>
									<span style={{ color: "var(--text-dim)" }}>{t("chat.hint-select")}</span>
									<Kbd>Tab</Kbd>
									<span style={{ color: "var(--text-dim)" }}>{t("chat.hint-complete")}</span>
									<Kbd>Esc</Kbd>
									<span style={{ color: "var(--text-dim)" }}>{t("chat.close")}</span>
								</span>
							</div>
							<div
								style={{
					...
					{atMenuOpen &&
						...
						(() => {
							...
							const matchCountLabel =
								atMatches.length === 1 ? t("chat.match") : t("chat.matches", { count: atMatches.length });
							// With a truncated index, local results are provisional — the
							// debounced server search over the full listing replaces them.
							const truncatedHint =
								fileIndex?.truncated && !serverResultInUse
									? atQuery.query
										? t("chat.searching-all")
										: t("chat.index-truncated")
									: "";
							return (
								<div
									style={{
				...
				{bashMode && (
					...
					>
						{t("chat.shell")} · {bashExcluded ? t("chat.output-local") : t("chat.output-model")}
					</div>
				)}
											...
											>
												{showModelFilter && (
													...
													>
														<input
															...
															placeholder={t("chat.filter-models")}
															aria-label={t("chat.filter-models")}
															autoFocus
															autoComplete="off"
															spellCheck={false}
															style={{
												...
												<div style={{ minHeight: 0, overflowY: "auto" }}>
													{modelsByProvider.length === 0 ? (
														...
														>
															{modelFilter.trim()
																? t("chat.no-matching-models")
																: modelError
																	? t("chat.models-load-failed")
																	: t("chat.no-available-models")}
														</div>
													) : (
														modelsByProvider.map((group, gi) => (
															<div key={group.provider}>
																...
																{group.options.map(opt => {
																	...
																	const isKbdActive =
																		flatIdx === modelActiveIndex && modelDropdownOpen;
																	return (
																		<button
					...
					>
						{isMobile && (
							<button
								...
								aria-label={t("chat.more-controls")}
						...
						>
							...
							{!isStreaming && onToolPresetChange && (
								<div ref={toolDropdownRef} style={{ position: "relative" }}>
									<button
										...
										disabled={isStreaming}
										title={`${t("chat.change-tool-preset")}: ${toolPresetLabel}`}
										aria-label={t("chat.change-tool-preset")}
										style={{
											display: "flex",
											alignItems: "center",
											justifyContent: "center",
											...
											{TOOL_PRESETS.map(lvl => {
												...
												const desc =
													lvl === "off"
														? t("chat.no-tools")
														: lvl === "default"
															? t("chat.built-in-tools", { count: 4 })
															: t("chat.all-built-in-tools");
												return (
													<button
														key={lvl}
														onClick={() => {
							...
							{!isStreaming && onCompact && (
								<div>
									<button
										...
										title={isCompacting ? t("chat.stop-compaction") : t("chat.compact-context")}
										aria-label={isCompacting ? t("chat.stop-compaction") : t("chat.compact-context")}
									>
										{isCompacting ? (
											<>
							...
							{onSoundToggle !== undefined && (
								<button
									...
									title={soundEnabled ? t("chat.disable-sound") : t("chat.enable-sound")}
									aria-label={soundEnabled ? t("chat.disable-sound") : t("chat.enable-sound")}
									style={{
										display: "flex",
										alignItems: "center",
										justifyContent: "center",
							...
							{isMobile && controlsMenuOpen && (
								<button
									...
									aria-label={t("chat.collapse-controls")}
									aria-expanded={true}
									onClick={() => {
										setToolDropdownOpen(false);
										setThinkingDropdownOpen(false);
