...
export async function GET(_req: Request, { params }: Params) {
  ...
  const isOmpConfigured = ompCredentials.some((c) => c.provider === provider);
  const displayName = modelRuntime.getProvider(provider)?.name ?? provider;
  const models = modelRuntime.getModels(provider).length;
  return NextResponse.json({
    provider,
    displayName,
    configured: status.configured || isOmpConfigured,
    source: isOmpConfigured ? "omp_agent_db" : status.source,
    models,
  });
}
...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    ...
    saveOmpApiKeyCredential(provider, apiKey.trim());
