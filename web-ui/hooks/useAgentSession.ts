...
export function useAgentSession(opts: UseAgentSessionOptions) {
		...
		const matchedModel = displayModel
			? modelList.find(
					m =>
						...
						(m.id === displayModel.modelId || displayModel.modelId.includes(m.id)),
				)
			: null;
	...
	const finishPromptWithoutStream = useCallback(
		async (sid: string | null = sessionIdRef.current, runId = promptRunIdRef.current) => {
	...
	const waitForBashSettlement = useCallback(
		async (sid: string) => {
			...
			while (bashRunningRef.current && bashRecoveryIdRef.current === recoveryId && sessionIdRef.current === sid) {
			...
			try {
				switch (commandName) {
					...
					case "models": {
						...
						return complete({
							handled: true,
							message: "Use the model selector dropdown below the chat box to change models",
						});
					}
					...
					case "settings": {
						return complete({
							handled: true,
							message: "Open Models & Settings from the sidebar bottom to configure settings",
						});
					}
					...
					case "resume": {
						return complete({
							handled: true,
							message: "Use the sidebar session tree to switch or resume sessions",
						});
					}
					...
					case "tree": {
						return complete({
							handled: true,
							message: "Use the branch switcher in the header to navigate session branches",
						});
					}
					...
					case "logout": {
						...
						return complete({
							handled: true,
							message: "Open Models & Settings from the sidebar to configure logins",
						});
					}
