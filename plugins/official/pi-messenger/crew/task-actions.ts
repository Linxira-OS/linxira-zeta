	...
	switch (action) {
		case "start": {
			if (task.milestone) {
				return {
					success: false,
					error: "milestone_not_startable",
					message: `Task ${taskId} is a milestone and cannot be started manually`,
				};
			}
		...
		case "unblock": {
			if (task.status !== "blocked") {
				return {
					...
					message: `Task ${taskId} is ${task.status}, not blocked`,
				};
			}
		...
		case "reset": {
			if (options?.isWorkerActive?.(taskId)) {
				return {
					success: false,
					error: "active_worker",
					message: `Cannot reset ${taskId} while its worker is active`,
				};
			}
		...
		case "cascade-reset": {
			...
			if (activeTask) {
				return {
					success: false,
					error: "active_worker",
					message: `Cannot reset ${activeTask.id} while its worker is active`,
				};
			}
