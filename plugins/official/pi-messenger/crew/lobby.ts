...
export function spawnLobbyWorker(cwd: string, promptOverride?: string, sessionModel?: string): LobbyWorker | null {
			...
			for (const line of lines) {
				...
				if (event) {
					...
					if (!worker.assignedTaskId) {
						...
						const budget =
							LOBBY_TOKEN_BUDGETS[currentConfig.coordination ?? "chatty"] ?? LOBBY_TOKEN_BUDGETS.chatty;
	...
	proc.on("close", exitCode => {
		...
		if (worker.aliveFile) {
			try {
				fs.unlinkSync(worker.aliveFile);
			} catch {}
		}
		if (worker.assignedTaskId) {
	...
	try {
		for (const f of fs.readdirSync(crewDir)) {
			if (f.startsWith("lobby-") && f.endsWith(".alive")) {
				try {
					fs.unlinkSync(path.join(crewDir, f));
				} catch {}
			}
		}
	} catch {}
