...
export async function execute(
	...
	const requestedConcurrency =
		concurrencyOverride ??
		(autonomous && isAutonomousForCwd(cwd) ? autonomousState.concurrency : config.concurrency.workers);
	...
	for (const lobbyWorker of lobbyWorkers) {
		...
		const prompt = buildWorkerPrompt(
			...
			others,
			skills,
			teamStore.buildTeamPromptContext(cwd, task),
		);
		store.updateTask(cwd, task.id, {
			status: "in_progress",
		...
		store.appendTaskProgress(
			...
			task.id,
			"system",
			`Assigned to lobby worker ${lobbyWorker.name} (attempt ${task.attempt_count + 1})`,
		);
	...
	const workerTasks = remainingTasks.map(task => {
		...
		const taskModel = resolveModel(task.model, params.model, roleModel, config.models?.worker, sessionModel);
		const others = readyTasks.filter(t => t.id !== task.id);
		const prompt = buildWorkerPrompt(
			task,
			prdLabel,
	...
	if (autonomous) {
		...
		if (signal?.aborted) {
			...
			const nextReady = store
				.getReadyTasks(cwd, { advisory: config.dependencies === "advisory" })
				.filter(t => !teamStore.taskNeedsApproval(t));
	...
	const progress = updatedPlan ? `${updatedPlan.completed_count}/${updatedPlan.task_count}` : "unknown";
	...
	if (finalNeedsApproval.length > 0)
	...
	const nextText =
		actionableNextReady.length > 0
			...
			: "";
	const continueText =
		autonomous && !signal?.aborted && actionableNextReady.length > 0
