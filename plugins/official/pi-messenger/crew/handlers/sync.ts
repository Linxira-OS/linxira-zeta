...
export async function execute(params: CrewParams, ctx: ExtensionContext, sessionModel?: string) {
	const cwd = ctx.cwd;
	const { target } = params;
	...
	const dependentTasks = allTasks.filter(t => t.depends_on.includes(target) && t.status === "todo");
	...
	const dependentOverview = dependentTasks
		.map(t => {
...
Follow the output format in your instructions.`;
	...
	for (const update of updates) {
		const matchingTask = dependentTasks.find(
			t => t.id === update.taskId || t.title.toLowerCase().includes(update.taskId.toLowerCase()),
		);
