...
export function buildDependencySection(
	cwd: string,
	task: Task,
	config: CrewConfig,
...
function formatActivityLine(event: FeedEvent): string {
	...
	if (event.type === "message") {
		...
		return event.preview ? `${time} ${event.agent} ✦ ${event.preview}` : `${time} ${event.agent} ✦`;
	}
...
`;
			for (const t of needsApproval)
				out += `- ${t.id}: ${t.title} — approve with \`pi_messenger({ action: "task.approve", id: "${t.id}" })\`\n`;
			out += "\n";
		}
...
`;
			for (const t of rejected)
				out += `- ${t.id}: ${t.title}${t.approval?.feedback ? ` — ${t.approval.feedback}` : ""} — revise with \`pi_messenger({ action: "task.revise", id: "${t.id}", prompt: "Address approval feedback" })\`\n`;
			out += "\n";
		}
	}
