...
Create a plan:
  ...
  pi_messenger({ action: "plan", prompt: "Scan the codebase for bugs" })   # Inline prompt`,
	...
	const waiting = tasks.filter(t => t.status === "todo" && !ready.some(r => r.id === t.id));
 ...
 */
export async function executeCrew(op: string, ctx: ExtensionContext) {
	const cwd = ctx.cwd;
