...
function setup(cwd: string, params: CrewParams) {
	...
	if (!teamStore.isValidProfileName(name)) {
		return result("Error: invalid team profile name. Use letters, numbers, underscore, or hyphen.", {
			mode: "team.setup",
			error: "invalid_name",
		});
	}
	...
	const customCharter =
		typeof params.message === "string" && params.message.trim().length > 0 ? params.message : undefined;
	const charter =
		customCharter || !existingCharter
...
function profileUse(cwd: string, params: CrewParams) {
	if (!params.name)
		return result("Error: name required for team.profile.use", { mode: "team.profile.use", error: "missing_name" });
...
function profileSave(cwd: string, params: CrewParams) {
	if (!params.name)
		return result("Error: name required for team.profile.save", { mode: "team.profile.save", error: "missing_name" });
	if (!teamStore.isValidProfileName(params.name)) {
		return result("Error: invalid team profile name. Use letters, numbers, underscore, or hyphen.", {
			mode: "team.profile.save",
			error: "invalid_name",
		});
	}
...
function charterCreate(cwd: string, params: CrewParams) {
	if (!params.name)
		return result("Error: name required for team.charter.create", {
			mode: "team.charter.create",
			error: "missing_name",
		});
	if (!params.message)
		return result("Error: message required for team.charter.create", {
			mode: "team.charter.create",
			error: "missing_message",
		});
	const charter = teamStore.writeCharter(cwd, params.name, params.message);
	return result(`Created charter for **${params.name}**.`, {
		mode: "team.charter.create",
		charter,
		team: teamStore.getActiveTeam(cwd),
	});
}

function charterUpdate(cwd: string, params: CrewParams) {
	if (!params.message)
		return result("Error: message required for team.charter.update", {
			mode: "team.charter.update",
			error: "missing_message",
		});
...
function memoryNote(cwd: string, params: CrewParams, state: MessengerState) {
	...
	if (!type)
		return result("Error: type must be one of decision, interface, risk, or handoff", {
			mode: "team.memory.note",
			error: "invalid_type",
		});
	if (!params.message)
		return result("Error: message required for team.memory.note", {
			mode: "team.memory.note",
			error: "missing_message",
		});
...
function memoryList(cwd: string, params: CrewParams) {
	...
	const text =
		entries.length > 0
			? entries
					.map(
						entry => `- ${entry.ts} [${entry.type}] ${entry.message}${entry.taskId ? ` (${entry.taskId})` : ""}`,
					)
					.join("\n")
...
function roles(cwd: string) {
	...
	const builtInNames = Object.keys(roles)
	...
	const note =
		"Team roles are labels and prompt context. Crew still runs tasks through Crew agents and does not launch pi-subagents.";
	...
	const nextAction =
		rejected.length > 0
			...
			: needsLead.length > 0
				...
				: team
					? "Next: plan or run Crew work with this Team context."
					: "Next: activate a sample profile, e.g. migration-squad, review-squad, or research-squad.";
