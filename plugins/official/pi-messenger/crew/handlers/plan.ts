...
function appendReviewToProgress(cwd: string, reviewNum: number, verdict: string, content: string): void {
	const progressPath = getProgressPath(cwd);
...
function pruneTransitiveDeps(cwd: string, taskIds: string[]): void {
	...
	for (const id of taskIds) {
		...
		const pruned = deps.filter(dep => !deps.some(other => other !== dep && isReachable(other, dep)));
...
export async function execute(
	...
	if (existingPlan) {
		...
		if (existingTasks.length > 0 && !prompt) {
			const planRef = existingPlan.prompt ? `"${store.getPlanLabel(existingPlan)}"` : existingPlan.prd;
			return result(
				`A plan already exists for ${planRef}.\n\nTo re-plan with a steering prompt:\n  pi_messenger({ action: "plan", prompt: "focus on..." })`,
				{
					mode: "plan",
					error: "plan_exists",
		...
		if (existingTasks.length > 0 && prompt) {
			...
			if (inProgress.length > 0) {
				return result(
					`Cannot re-plan: ${inProgress.length} task(s) in progress (${inProgress.map(t => t.id).join(", ")}). Stop or complete them first.`,
					{
						mode: "plan",
	...
	const runLabel = isPromptBased ? (prompt!.length > 60 ? prompt!.slice(0, 57) + "..." : prompt!) : prdPath;
	...
	notify(
		ctx,
		`Planning started: ${isPromptBased ? runLabel : path.basename(prdPath)} (${maxPasses} pass${maxPasses === 1 ? "" : "es"})`,
		"info",
	);
	reportProgress();
	...
	for (let pass = 1; pass <= maxPasses; pass++) {
		...
		advancePhase(
			...
			"plan.review.done",
			prdPath,
			`review ${pass}: ${lastVerdict.verdict}`,
			pass,
		);
		reportProgress();
	...
	const existingByTitle = new Map(store.getTasks(cwd).map(t => [t.title.toLowerCase(), t.id] as const));
	...
	pruneTransitiveDeps(
		cwd,
		plannedTasks.map(t => t.id),
	);

	store.setPlanSpec(cwd, lastPlannerOutput);

	const taskList = plannedTasks
		.map(t => {
...
function formatTeamForPlanner(
	...
	const roleLines = Object.values(roles)
		...
		.map(
			role =>
				`  ${role.name}${role.description ? ` — ${role.description}` : ""}${role.model ? ` (model: ${role.model})` : ""}`,
		);
	const approvalLine =
...
function tasksJsonFormatHint(skills: CrewSkillInfo[], teamActive: boolean): string {
	...
	if (skills.length > 0)
		fields.push(
			"optionally skills (array of skill names from the Available Skills list that are relevant to the task)",
		);
...
function extractPlanSections(output: string): PlanSections | null {
	...
	const s1Body = section(s1Start, s2Start)
		.replace(/^##\s+1\.\s+.+\n?/, "")
		.trim();
	const s2Body = section(s2Start, s3Start)
		.replace(/^##\s+2\.\s+.+\n?/, "")
		.trim();
	const s3Body = section(s3Start, s4Start)
		.replace(/^##\s+3\.\s+.+\n?/, "")
		.trim();
	const s4Body = section(s4Start, end)
		.replace(/^##\s+4\.\s+.+\n?/, "")
		.trim();
	...
	try {
		...
		const tasks = parsed
			...
			.map(t => ({
				...
				riskLabels: Array.isArray(t.riskLabels)
					? t.riskLabels.filter((s: unknown) => typeof s === "string")
					: undefined,
			}));
...
function parseTasksFromOutput(output: string): ParsedTask[] {
	...
	while ((match = taskRegex.exec(output)) !== null) {
		...
		const description = body.replace(/Dependencies?:\s*.+?(?:\n|$)/i, "").trim();
