...
import { getArtifactPaths, ensureArtifactsDir, writeArtifact, writeMetadata, appendJsonl } from "./utils/artifacts.ts";
...
function discoverWorkerName(pid: number | undefined, registryDir: string | undefined): string | null {
...
export async function spawnAgents(tasks: AgentTask[], cwd: string, options: SpawnOptions = {}): Promise<AgentResult[]> {
	...
	while (queue.length > 0 || running.length > 0) {
		...
		while (running.length < autonomousState.concurrency && queue.length > 0) {
			...
			const promise = runAgent(task, index, cwd, agents, config, runId, artifactsDir, options).then(result => {
				results.push(result);
...
async function runAgent(
	...
	let artifactPaths = config.artifacts.enabled ? getArtifactPaths(artifactsDir, runId, task.agent, index) : undefined;
	...
	return new Promise(resolve => {
		...
		const thinking = resolveThinking(config.thinking?.[role], agentConfig?.thinking);
		...
		const workerFlag = role === "worker" ? { PI_CREW_WORKER: "1", PI_AGENT_NAME: workerName } : {};
		const env =
			Object.keys(envOverrides).length > 0 || role === "worker"
			...
			try {
				...
				for (const line of lines) {
					...
					if (event) {
						...
						if (artifactPaths) {
							try {
								appendJsonl(artifactPaths.jsonlPath, JSON.stringify(compactEventForArtifact(event)));
							} catch {
								artifactPaths = undefined;
							}
						}
