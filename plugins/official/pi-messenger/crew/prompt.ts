...
import {
	...
	buildCoordinationInstructions,
} from "./handlers/coordination.ts";
...
function buildTeamSection(task: Task, teamContext: TeamPromptContext | undefined): string | null {
	...
	if (teamContext.team) {
		const profileText =
			teamContext.profile?.name && teamContext.profile.name !== teamContext.team.name
	...
	if (task.role || teamContext.role) {
		...
		if (teamContext.role?.skills && teamContext.role.skills.length > 0)
			section += `Role skills: ${teamContext.role.skills.join(", ")}\n`;
		if (teamContext.role && isNonEditingTeamRole(teamContext.role.name)) {
			section +=
				"This is a non-editing Team role. Do not edit files; report findings, plans, decisions, or handoff notes instead.\n";
		}
		if (teamContext.role?.prompt) {
			const prompt =
				teamContext.role.prompt.length > 2000
	...
	if (teamContext.charter) {
		const charter =
			teamContext.charter.length > 2500
