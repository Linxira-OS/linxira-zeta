...
export function cleanupBlockFiles(cwd: string, taskId: string): void {
	try {
		fs.unlinkSync(path.join(getBlocksDir(cwd), `${taskId}.md`));
	} catch {}
}
...
export function getPlanLabel(plan: Plan, maxLen = 60): string {
	...
	return plan.prompt.length > maxLen ? plan.prompt.slice(0, maxLen - 3) + "..." : plan.prompt;
}
...
export function createTask(
	...
	const specContent = description ? `# ${title}\n\n${description}\n` : `# ${title}\n\n*Spec pending*\n`;
...
function normalizeApproval(raw: unknown): TaskApproval | undefined {
	...
	const status: TaskApprovalDecisionStatus =
...
function normalizeTask(raw: Task): Task {
	...
	return {
		...
		risk_labels: normalizeRiskLabels(
			Array.isArray(raw.risk_labels) ? raw.risk_labels.filter(s => typeof s === "string") : undefined,
		),
		approval,
...
export function deleteTask(cwd: string, taskId: string): boolean {
	...
	for (const ext of [".json", ".md", ".progress.md"]) {
		try {
			fs.unlinkSync(path.join(tasksDir, `${taskId}${ext}`));
		} catch {}
	}
...
export function completeTask(cwd: string, taskId: string, summary: string, evidence?: TaskEvidence): Task | null {
	const task = getTask(cwd, taskId);
