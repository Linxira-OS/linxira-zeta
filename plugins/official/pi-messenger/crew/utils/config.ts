	...
	for (const source of sources) {
		...
		for (const key of Object.keys(src)) {
			...
			if (sourceVal && typeof sourceVal === "object" && !Array.isArray(sourceVal)) {
				const base =
					targetVal && typeof targetVal === "object" && !Array.isArray(targetVal) ? (targetVal as object) : {};
...
export function getTruncationForRole(config: CrewConfig, role: string): MaxOutputConfig {
	switch (role) {
		case "planner":
			return config.truncation.planners;
		case "worker":
			return config.truncation.workers;
		case "reviewer":
			return config.truncation.reviewers;
		case "analyst":
			return config.truncation.analysts;
		default:
			return config.truncation.workers;
	}
}
