	...
	"migration-squad": {
		...
		roles: {
			...
			worker: {
				...
				prompt:
					"Before editing, confirm the migration plan, rollback path, and affected interfaces are clear. Keep changes narrow and record risky decisions in Team memory.",
			},
		...
		approval: {
			mode: "risk-labels",
			labels: ["database", "migration", "destructive", "api-contract", "auth", "payment"],
		},
...
function normalizeProfile(raw: unknown, filePath: string, expectedName?: string): TeamProfile {
	...
	if (raw.roles !== undefined) {
		...
		for (const [name, value] of Object.entries(raw.roles)) {
			...
			if (!isValidProfileName(name) || !isRecord(value)) {
				throw new Error(
					`Invalid Team profile JSON at ${filePath}: role ${name} must be an object with a filename-safe name`,
				);
			}
	...
	if (raw.approval !== undefined) {
		if (!isRecord(raw.approval))
			throw new Error(`Invalid Team profile JSON at ${filePath}: approval must be an object`);
		const mode = raw.approval.mode ?? "risk-labels";
		if (mode !== "off" && mode !== "risk-labels")
			throw new Error(`Invalid Team profile JSON at ${filePath}: approval.mode is invalid`);
		...
		if (Array.isArray(raw.approval.labels)) {
			for (const label of raw.approval.labels) {
				if (typeof label !== "string" || label.trim().length === 0) {
					throw new Error(
						`Invalid Team profile JSON at ${filePath}: approval.labels entries must be non-empty strings`,
					);
				}
			}
		}
	...
	if (raw.memory !== undefined) {
		...
		if (Array.isArray(raw.memory.inject)) {
			for (const type of raw.memory.inject) {
				if (!isTeamMemoryType(type)) {
					throw new Error(
						`Invalid Team profile JSON at ${filePath}: memory.inject entries must be one of ${TEAM_MEMORY_TYPES.join(", ")}`,
					);
				}
			}
		}
		profile.memory = {
			...
			...(typeof raw.memory.maxCharsPerType === "number" && Number.isFinite(raw.memory.maxCharsPerType)
				? { maxCharsPerType: raw.memory.maxCharsPerType }
				: {}),
		};
	}
...
export function useProfile(
	...
	name: string,
	homeDir?: string,
): { team: TeamState; profile: TeamProfile; created: boolean } {
	if (!isValidProfileName(name)) throw new Error(`Invalid team profile name: ${name}`);
...
export function noteMemory(
	...
	fs.appendFileSync(
		teamPath(cwd, MEMORY_FILES[type]),
		`- ${entry.ts} (${agent}${taskId ? `, ${taskId}` : ""}): ${message}\n`,
	);
	return entry;
}
...
export function approvalForTask(
	cwd: string,
