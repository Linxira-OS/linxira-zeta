...
import { spawnWorkerForTask, getAvailableLobbyWorkers, assignTaskToLobbyWorker } from "./lobby.ts";
...
export function spawnWorkersForReadyTasks(cwd: string, maxWorkers: number, sessionModel?: string): SpawnResult {
	...
	for (const lw of lobby) {
		...
		store.appendTaskProgress(
			...
			task.id,
			"system",
			`Assigned to lobby worker ${lw.name} (attempt ${task.attempt_count + 1})`,
		);
...
export function spawnSingleWorker(cwd: string, taskId: string, sessionModel?: string): { name: string } | null {
	const plan = store.getPlan(cwd);
	if (!plan) return null;
