...
import type { ExtensionContext } from "@linxiraos/zeta";
import {
	type MessengerState,
	type Dirs,
...
export function notRegisteredError() {
	return result('Not registered. Use pi_messenger({ action: "join" }) to join the agent mesh first.', {
		mode: "error",
...
export async function executeLeave(state: MessengerState, dirs: Dirs, ctx: ExtensionContext) {
	if (!state.registered) {
	...
	try {
		...
		const message = error instanceof Error ? error.message : String(error);
		return result(`Could not leave pi-messenger: ${message}`, { mode: "leave", error: "unregister_failed", message });
	}
...
export function executeList(state: MessengerState, dirs: Dirs, cwd: string, config?: { stuckThreshold?: number }) {
	...
	lines.push(
		formatAgentLine(
			...
			true,
			agentHasTask(state.agentName, allClaims, crewStore.getTasks(cwd)),
		),
	);
...
export function executeSend(
	...
	if (!message) {
		return result("Error: message is required when sending.", { mode: "send", error: "missing_message" });
	}
	...
	if (broadcast) {
		if (process.env.PI_CREW_WORKER) {
			...
			return result(`Broadcast logged. (${remaining} message${remaining === 1 ? "" : "s"} remaining)`, {
				mode: "send",
				sent: ["feed"],
				failed: [],
			});
		...
		if (recipients.length === 0) {
			return result("No active agents to broadcast to.", { mode: "send", error: "no_recipients" });
		}
	} else if (to) {
		recipients = [...new Set(Array.isArray(to) ? to : [to])];
		if (recipients.length === 0) {
			return result("Error: recipient list cannot be empty.", { mode: "send", error: "empty_recipients" });
		}
	} else {
		return result("Error: specify 'to' or 'broadcast: true'.", { mode: "send", error: "missing_recipient" });
	}
	...
	if (sent.length === 0) {
		...
		return result(`Failed to send: ${failedStr}`, { mode: "send", error: "all_failed", sent: [], failed });
	}
...
export function executeReserve(
	...
	if (patterns.length === 0) {
		return result("Error: at least one pattern required.", { mode: "reserve", error: "empty_patterns" });
	}
...
export function executeRelease(state: MessengerState, dirs: Dirs, ctx: ExtensionContext, release: string[] | true) {
	if (!state.registered) {
		return notRegisteredError();
	}
	...
	if (release === true) {
		...
		return result(released.length > 0 ? `Released all: ${released.join(", ")}` : "No reservations to release.", {
			mode: "release",
			released,
		});
	}
...
export function executeSetSpec(state: MessengerState, dirs: Dirs, ctx: ExtensionContext, specPath: string) {
	const cwd = ctx.cwd;
...
export async function executeComplete(
	...
	if (store.isCompleteAlreadyCompleted(completeResult)) {
		return result(`Error: ${taskId} was already completed by ${completeResult.completion.completedBy}.${warning}`, {
			mode: "complete",
	...
	if (store.isCompleteNotYours(completeResult)) {
		return result(`Error: ${taskId} is claimed by ${completeResult.claimedBy}, not you.${warning}`, {
			mode: "complete",
			error: "not_your_claim",
			taskId,
...
export function executeSwarm(state: MessengerState, dirs: Dirs, cwd: string, specPath?: string) {
	...
	for (const display of Array.from(allSpecs).sort((a, b) => a.localeCompare(b))) {
		...
		const idle = specAgentList.filter(name => !Object.values(specClaims).some(c => c.agent === name));
...
export function executeSetStatus(state: MessengerState, dirs: Dirs, ctx: ExtensionContext, message?: string) {
	if (!state.registered) {
		return notRegisteredError();
	}
	...
	if (!message || message.trim() === "") {
		...
		store.updateRegistration(state, dirs, ctx);
		return result("Custom status cleared. Auto-status will resume.", { mode: "set_status", cleared: true });
	}

	state.statusMessage = message.trim();
	state.customStatus = true;
	store.updateRegistration(state, dirs, ctx);
	return result(`Status set to: ${state.statusMessage}`, { mode: "set_status", message: state.statusMessage });
}

export function executeFeed(cwd: string, limit?: number, crewEventsInFeed: boolean = true) {
	...
	if (events.length === 0) {
		return result("# Activity Feed\n\nNo activity yet.", { mode: "feed", events: [] });
	}
...
export function executeWhois(
	...
	if (!agent) {
		...
		return result(`Agent "${name}" not found or not active.`, { mode: "whois", error: "not_found", name });
	}
...
function formatWhoisOutput(agent: AgentRegistration, isSelf: boolean, dirs: Dirs, cwd: string, thresholdMs: number) {
	const allClaims = store.getClaims(dirs);
...
export function executeAutoRegisterPath(action: "add" | "remove" | "list", cwd: string) {
