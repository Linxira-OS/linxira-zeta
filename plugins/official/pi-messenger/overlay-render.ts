...
import type { Theme } from "@linxiraos/zeta";
import {
	formatDuration,
	formatRelativeTime,
...
export function renderStatusBar(theme: Theme, cwd: string, width: number, tasks?: Task[]): string {
	...
	if (isPlanningForCwd(cwd)) {
		...
		return truncateToWidth(
			`${label} ${planningState.pass}/${planningState.maxPasses} │ ${planningState.phase} │ ${updated}${workerNote} │ ${crewConfig.dependencies} │ ${coordLevel}`,
			width,
		);
	}
	...
	if (!plan) {
		...
		return truncateToWidth(
			`No active plan │ ⚙ ${liveCount}/${autonomousState.concurrency} workers${teamText}`,
			width,
		);
	}
...
export function renderWorkersSection(theme: Theme, cwd: string, width: number, maxLines: number): string[] {
	...
	for (const info of workers) {
		...
		const tokens =
			info.progress.tokens > 1000 ? `${(info.progress.tokens / 1000).toFixed(0)}k` : `${info.progress.tokens}`;
	...
	for (let i = 0; i < taskSnapshot.length; i++) {
		lines.push(
			renderTaskLine(
				...
				width,
				liveWorkers.get(taskSnapshot[i].id),
			),
		);
	}
...
export function renderTaskSummary(theme: Theme, cwd: string, width: number, height: number, tasks?: Task[]): string[] {
	...
	const line2 =
		activeNames.length > 0 ? truncateToWidth(theme.fg("dim", `  Active: ${activeNames.join(", ")}`), width) : "";
	const lines = [line1];
...
export function renderPlanningState(theme: Theme, cwd: string, width: number, height: number): string[] {
	...
	lines.push(
		`  Pass: ${planningState.pass}/${planningState.maxPasses}  │  Phase: ${planningState.phase}  │  ${updated}`,
	);

	if (stalled) {
		const staleFor = ageMs === null ? "unknown" : formatDuration(ageMs);
		lines.push(
			theme.fg("warning", `  Health: stalled (${staleFor}, timeout ${formatDuration(PLANNING_STALE_TIMEOUT_MS)})`),
		);
	}
...
export function renderLegend(
	...
	return truncateToWidth(
		theme.fg("dim", appendUniversalHints(`m:Chat  v:${coordHint(cwd)}  +/-:Wkrs  Esc:Close`)),
		width,
	);
}
...
export function renderDetailView(
	...
	if (live) {
		...
		lines.push(
			`Status: ${task.status}  │  Attempts: ${task.attempt_count}  │  Created: ${formatRelativeTime(task.created_at)}${typeText}${assignedText}`,
		);
	}
	lines.push("");
	...
	if (live) {
		...
		if (task.last_review) {
			...
			lines.push(
				`Last Review: ${icon} ${task.last_review.verdict} (${formatRelativeTime(task.last_review.reviewed_at)})`,
			);
...
function renderTaskLine(
	...
	return truncateToWidth(
		`${select}${coloredIcon} ${task.id}  ${labelText}${task.title}${theme.fg("dim", suffix)}`,
		width,
	);
}
