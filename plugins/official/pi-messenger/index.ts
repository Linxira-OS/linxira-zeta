...
export default function piMessengerExtension(pi: ExtensionAPI) {
	...
	function updateStatus(ctx: ExtensionContext): void {
		try {
			...
			const planningStr = isPlanningForCwd(planningCwd)
				? theme.fg(
						"warning",
			...
			const activityStr =
				!planningStr && state.activity.currentActivity
  ...
  pi_messenger({ action: "team.status" })                       → Team summary`,
		...
		parameters: Type.Object({
			...
			prompt: Type.Optional(
				Type.String({
					description: "Inline prompt for plan action, or revision instructions for task.revise/task.revise-tree",
				}),
			),
			id: Type.Optional(Type.String({ description: "Task ID (task-N format)" })),
			taskId: Type.Optional(
				Type.String({ description: "Swarm task ID (e.g., TASK-01) - for action-based claim/unclaim/complete" }),
			),
			title: Type.Optional(Type.String({ description: "Title for task.create" })),
			dependsOn: Type.Optional(
				Type.Array(Type.String(), { description: "Task IDs this task depends on (for task.create)" }),
			),
			role: Type.Optional(
				Type.String({
					description:
						"Team role for task creation/planning (built-ins follow packaged pi-subagents vocabulary; profiles may define more)",
				}),
			),
			...
			type: Type.Optional(
				StringEnum(["plan", "impl", "decision", "interface", "risk", "handoff"], {
					description: "Review type, or Team memory type for team.memory.*",
				}),
			),
			autoWork: Type.Optional(
				Type.Boolean({
					description:
						"Auto-start autonomous work after plan completes (default: true, pass false to review plan first)",
				}),
			),
			...
			name: Type.Optional(
				Type.String({ description: "Name for rename action or Team setup/profile/charter commands" }),
			),
			...
			message: Type.Optional(
				Type.String({ description: "Message to send, Team setup/charter text, or Team memory note" }),
			),
			replyTo: Type.Optional(Type.String({ description: "Message ID if this is a reply" })),
			reason: Type.Optional(
				Type.String({ description: "Reason for reservation, claim, task block, or approval rejection feedback" }),
			),
			autoRegisterPath: Type.Optional(
				StringEnum(["add", "remove", "list"], {
		...
		async execute(_toolCallId, rawParams, signal, _onUpdate, ctx) {
			...
			const result = await executeCrewAction(
				...
				(type, data) => pi.appendEntry(type, data),
				{
					...
					crewEventsInFeed: config.crewEventsInFeed,
					nameTheme,
	...
	function maybeAutoOpenCrewOverlay(ctx: ExtensionContext): void {
		...
		const autonomousPending = config.autoOverlay && autonomousState.active && autonomousState.autoOverlayPending;

		const planningPending = config.autoOverlayPlanning ? getPlanningOverlayPending(cwd) : null;
		...
		const planningRunId = planningPending ? (consumePlanningOverlayPending(cwd)?.runId ?? null) : null;
	...
	pi.on("agent_end", async (_event, ctx) => {
		...
		const continueSignature = `${cwd}:${autonomousState.waveNumber}:${startableTasks
			.map(task => task.id)
			.sort()
			.join(",")}`;
