...
export function formatFeedLine(event: FeedEvent): string {
	...
	const preview = rawPreview ? (rawPreview.length > 90 ? rawPreview.slice(0, 87) + "..." : rawPreview) : "";
	...
	switch (sanitized.type) {
		...
		case "task.done":
			...
			break;
		case "task.review":
			line += withPreview(` reviewed ${target}`);
			break;
		case "task.block":
		...
		case "task.revise-tree":
			...
			break;
		case "task.approve":
			line += withPreview(` approved ${target}`);
			break;
		case "task.reject":
			line += withPreview(` rejected ${target}`);
			break;
		case "plan.start":
...
export function logFeedEvent(cwd: string, agent: string, type: FeedEventType, target?: string, preview?: string): void {
	appendFeedEvent(cwd, {
