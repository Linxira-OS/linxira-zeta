...
describe("Team task approval gates", () => {
	...
	it("surfaces rejected tasks as revision work", async () => {
		...
		const start = await taskHandler.execute(
			"start",
			{ id: rejected.id },
			createState("Worker"),
			createMockContext(cwd),
		...
		expect(ready.content[0].text).toContain(
			'pi_messenger({ action: "task.revise", id: "task-1", prompt: "Address approval feedback" })',
		);
	});
	...
	it("approves gated tasks so they can be started", async () => {
		...
		const approval = await taskHandler.execute(
			"approve",
			{ id: gated.id },
			createState("Lead"),
			createMockContext(cwd),
	...
	it("rejects approval changes after work starts or finishes", async () => {
		...
		const activeReject = await taskHandler.execute(
			"reject",
			{ id: active.id, reason: "too late" },
			createState("Lead"),
			createMockContext(cwd),
		);
		...
		const doneReject = await taskHandler.execute(
			"reject",
			{ id: done.id, reason: "too late" },
			createState("Lead"),
			createMockContext(cwd),
		);
	...
	it("reports approval-gated tasks unlocked by task completion", async () => {
		...
		const done = await taskHandler.execute(
			"done",
			{ id: first.id, summary: "Done" },
			createState("Worker"),
			createMockContext(cwd),
		);
