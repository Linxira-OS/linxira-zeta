...
describe("plan with prompt (re-plan)", () => {
	...
	it("re-plans when prompt provided with existing tasks", async () => {
		...
		const r = await planHandler.execute({ action: "plan", prompt: "focus on performance" }, mockCtx, "agent");
	...
	it("rejects re-plan when tasks are in_progress", async () => {
		...
		const r = await planHandler.execute({ action: "plan", prompt: "re-plan" }, mockCtx, "agent");
	...
	it("injects prompt into planning-progress.md Notes section", async () => {
		...
		await planHandler.execute({ action: "plan", prompt: "ignore auth" }, mockCtx, "agent");
...
describe("plan with prompt as spec", () => {
	...
	it("auto-discovered PRD takes priority over prompt", async () => {
		...
		const r = await planHandler.execute({ action: "plan", prompt: "focus on errors" }, mockCtx, "agent");
	...
	it("prompt-based plan shows correct success label", async () => {
		...
		const r = await planHandler.execute({ action: "plan", prompt: "Scan for bugs" }, mockCtx, "agent");
...
describe("plan transitive dependency pruning", () => {
	...
	it("prunes diamond transitive dependencies", async () => {
		...
		const byTitle = new Map(tasks.map(t => [t.title, t]));
		expect(byTitle.get("Task D")?.depends_on).toEqual([byTitle.get("Task B")!.id, byTitle.get("Task C")!.id]);
	});
...
describe("getPlanLabel", () => {
	...
	it("respects custom maxLen", () => {
		const plan: Plan = {
			prd: "(prompt)",
			prompt: "Short prompt that is long enough",
			created_at: "",
			updated_at: "",
			task_count: 0,
			completed_count: 0,
...
describe("plan with autoWork", () => {
	...
	it("sets pendingAutoWork when autoWork is true", async () => {
		...
		const r = await planHandler.execute({ action: "plan", autoWork: true }, mockCtx, "agent");
	...
	it("does not set pendingAutoWork when autoWork is false", async () => {
		...
		const r = await planHandler.execute({ action: "plan", autoWork: false }, mockCtx, "agent");
