...
describe("team command routing", () => {
	...
	it("sets up a profile and starter charter in one command", async () => {
		...
		const custom = await run("team.setup", {
			name: "review-squad",
	...
	it("routes team commands", async () => {
		...
		expect(
			(await run("team.charter.create", { name: "migration-squad", message: "Ship safely." })).details.mode,
		).toBe("team.charter.create");
		expect((await run("team.charter.show")).details.mode).toBe("team.charter.show");
		expect((await run("team.charter.update", { message: "Prefer reviews." })).details.mode).toBe(
			"team.charter.update",
		);
		expect((await run("team.memory.note", { type: "decision", message: "Use JSON profiles." })).details.mode).toBe(
			"team.memory.note",
		);
