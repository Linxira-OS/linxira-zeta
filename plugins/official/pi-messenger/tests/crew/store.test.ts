...
describe("crew/store", () => {
	...
	describe("task CRUD", () => {
		...
		it("loads pre-Team task JSON without role/risk/approval fields", () => {
			store.createPlan(cwd, "docs/PRD.md");
			fs.writeFileSync(
				path.join(dirs.tasksDir, "task-1.json"),
				JSON.stringify({
					id: "task-1",
					title: "Legacy task",
					status: "todo",
		...
		it("normalizes persisted approval fields explicitly", () => {
			...
			fs.writeFileSync(
				path.join(dirs.tasksDir, "task-2.json"),
				JSON.stringify({
					id: "task-2",
	...
	describe("task lifecycle", () => {
		...
		it("completeTask: in_progress -> done and updates plan completed_count", () => {
			...
			const completed = store.completeTask(cwd, task.id, "Implemented feature", {
				commits: ["abc123"],
				tests: ["npm test"],
			});
	...
	describe("task progress", () => {
		...
		it("appendTaskProgress appends multiple entries without overwriting", () => {
			...
			const content = fs.readFileSync(path.join(dirs.tasksDir, `${task.id}.progress.md`), "utf-8");
