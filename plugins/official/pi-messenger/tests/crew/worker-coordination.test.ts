...
describe("buildDependencySection", () => {
	...
	it("in advisory mode renders done/in-progress/todo dependency states and non-blocking guidance", () => {
		writeTask(dirs.tasksDir, {
			id: "task-1",
			title: "Types and Config",
			status: "done",
			summary: "Created types.ts with ObservationConfig",
		});
...
describe("buildCoordinationContext", () => {
	...
	it("with level chatty includes concurrent tasks, recent activity, and ready tasks", () => {
		...
		writeFeedEvents(dirs.cwd, [makeEvent("2026-01-01T22:14:00Z", "task.done", "EpicGrove", "task-1", "Done")]);
	...
	it("chatty ready tasks separates approval-gated and rejected tasks", () => {
		...
		const result = buildCoordinationContext(
			dirs.cwd,
		...
		expect(result).toContain(
			'pi_messenger({ action: "task.revise", id: "task-5", prompt: "Address approval feedback" })',
		);
	});
	...
	it("formats message events in recent activity with direction indicators", () => {
		writeFeedEvents(dirs.cwd, [
			...
			{
				...makeEvent("2026-01-01T22:11:00Z", "message", "EpicGrove"),
				target: "OakBear",
				preview: "Need User type from schema",
			},
			{
				...makeEvent("2026-01-01T22:12:00Z", "message", "OakBear"),
				preview: "Completed task-2: schema.ts exports User, Session",
			},
		]);
...
describe("config coordination", () => {
	...
	it("default config has coordination: chatty with message budgets", async () => {
		const { loadCrewConfig } = await (async () => {
			vi.resetModules();
...
describe("executeSend broadcast filtering", () => {
	...
	it("worker broadcast logs to feed only", () => {
		...
		const result = executeSend(state as any, messageDirs as any, dirs.cwd, undefined, true, "Worker update");
	...
	it("non-worker broadcast delivers to inbox recipients", () => {
		...
		executeSend(state as any, messageDirs as any, dirs.cwd, undefined, true, "Team-wide update");
	...
	it("worker direct message still delivers", () => {
		...
		executeSend(state as any, messageDirs as any, dirs.cwd, "OakBear", undefined, "Need your input");
	...
	it("worker broadcast increments message budget usage", () => {
		...
		const first = executeSend(state as any, messageDirs as any, dirs.cwd, undefined, true, "First broadcast");
		const second = executeSend(state as any, messageDirs as any, dirs.cwd, undefined, true, "Second broadcast");
