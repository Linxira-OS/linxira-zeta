...
describe("plan with skills", () => {
	...
	it("handles tasks-json with no skills field gracefully", async () => {
		spawnAgents.mockResolvedValue([
			{
				exitCode: 0,
				output: plannerOutputWithSkills([{ title: "Task A", description: "Do A", dependsOn: [] }]),
				error: null,
				progress: { toolCallCount: 0, tokens: 0 },
			},
		]);
	...
	it("skips invalid tasks-json items without discarding the block", async () => {
		spawnAgents.mockResolvedValue([
			{
				exitCode: 0,
				output: plannerOutputWithSkills([null, { title: "Task A", description: "Do A", dependsOn: [] }]),
				error: null,
				progress: { toolCallCount: 0, tokens: 0 },
			},
		]);
	...
	it("shows default approval labels in planner prompt when profile omits labels", async () => {
		...
		expect(plannerPrompt).toContain(
			"High-risk labels requiring lead approval for editing roles: security, database, api-contract, destructive, migration, auth, payment",
		);
	});
