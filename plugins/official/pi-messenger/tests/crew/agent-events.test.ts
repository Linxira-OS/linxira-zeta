		...
		proc.stdout.emit(
			...
			`${JSON.stringify({
				...
				assistantMessageEvent: {
					type: "text_delta",
					delta: "ok",
					partial: { role: "assistant", content: [{ type: "text", text: repeated }] },
				},
		...
		proc.stdout.emit(
			...
			`${JSON.stringify({
				...
				error: {
					...
					message:
						"Third-party apps now draw from your extra usage. Add more at claude.ai/settings/usage and keep going.",
				},
			})}\n`,
		);
