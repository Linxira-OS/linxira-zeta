...
import { discoverCrewAgents, discoverCrewSkills } from "../../../crew/utils/discover.ts";
...
`,
		...
		writeAgent(
			path.join(projectAgentsDir, "crew-reviewer.md"),
			`---
name: crew-reviewer
...
`,
	...
	it("returns extension agents when project directory is missing", () => {
		writeAgent(
			path.join(extensionAgentsDir, "crew-planner.md"),
			`---
name: crew-planner
description: Planner
crewRole: planner
...
`,
	...
	it("parses thinking from frontmatter", () => {
		writeAgent(
			path.join(extensionAgentsDir, "crew-thinker.md"),
			`---
name: crew-thinker
...
`,
	...
	it("thinking defaults to undefined when not specified", () => {
		writeAgent(
			path.join(extensionAgentsDir, "crew-simple.md"),
			`---
name: crew-simple
...
`,
	...
	it("parses frontmatter fields", () => {
		writeAgent(
			path.join(extensionAgentsDir, "crew-analyst.md"),
			`---
name: crew-analyst
description: Analyst
...
`,
	...
	it("discovers user skills from directory/SKILL.md format", () => {
		writeSkill(
			path.join(userSkillsDir, "react-best-practices", "SKILL.md"),
			`---
...
`,
	...
	it("extension skills override user skills with the same name", () => {
		writeSkill(
			path.join(userSkillsDir, "api-design", "SKILL.md"),
			`---
name: api-design
...
`,
		...
		writeSkill(
			path.join(extensionSkillsDir, "api-design.md"),
			`---
name: api-design
...
`,
		...
		writeSkill(
			path.join(extensionSkillsDir, "coordination.md"),
			`---
name: coordination
...
`,
		...
		writeSkill(
			path.join(projectSkillsDir, "react-patterns.md"),
			`---
name: react-patterns
...
`,
	...
	it("skips files without name or description in frontmatter", () => {
		writeSkill(
			path.join(extensionSkillsDir, "bad.md"),
			`---
name: incomplete
---
...
`,
		...
		writeSkill(
			path.join(extensionSkillsDir, "good.md"),
			`---
name: good-skill
...
`,
	...
	it("returns absolute paths for all skills", () => {
		writeSkill(
			path.join(extensionSkillsDir, "ext.md"),
			`---
name: ext-skill
...
`,
		...
		writeSkill(
			path.join(userSkillsDir, "usr", "SKILL.md"),
			`---
name: usr-skill
...
`,
	...
	it("truncates multiline descriptions to first line", () => {
		writeSkill(
			path.join(extensionSkillsDir, "multi.md"),
			`---
name: multi-desc
