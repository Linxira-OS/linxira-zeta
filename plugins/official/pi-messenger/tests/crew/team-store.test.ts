...
describe("crew/team store", () => {
	...
	it("allows non-packaged roles from saved profiles", () => {
		...
		teamStore.saveProfile(
			{ name: "custom-squad", roles: { simplifier: { description: "Project-specific simplifier" } } },
			home,
		);
	...
	it("rejects saved profiles with invalid consumed fields", () => {
		...
		fs.writeFileSync(
			path.join(profilesDir, "bad-label.json"),
			JSON.stringify({
				name: "bad-label",
				approval: { mode: "risk-labels", labels: ["auth", ""] },
			}),
		...
		fs.writeFileSync(
			path.join(profilesDir, "bad-memory.json"),
			JSON.stringify({
				name: "bad-memory",
		...
		expect(() => teamStore.useProfile(cwd, "bad-label")).toThrow(
			/approval\.labels entries must be non-empty strings/,
		);
	...
	it("canonicalizes packaged role casing from saved profiles", () => {
		fs.mkdirSync(profilesDir, { recursive: true });
		fs.writeFileSync(
			path.join(profilesDir, "case-team.json"),
			JSON.stringify({
				name: "case-team",
