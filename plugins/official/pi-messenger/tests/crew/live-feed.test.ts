...
describe("renderFeedSection (pure formatter)", () => {
	...
	it("renders all events as normal when lastSeenTs is null (first open)", () => {
		const events = [makeEvent("2026-01-01T00:00:00Z", "task.done"), makeEvent("2026-01-01T00:01:00Z", "task.start")];
	...
	it("dims all events when lastSeenTs equals latest event ts", () => {
		const events = [makeEvent("2026-01-01T00:01:00Z", "join")];
		...
		const events: FeedEvent[] = [
			{
				...
				target: "OakBear",
				preview:
					"I've finished implementing the JWT middleware and exported AuthMiddleware, validateToken, and refreshTokenRotation from src/auth/middleware.ts",
			},
		];
		const lines = renderFeedSection(mockTheme as any, events, 80, null);
