...
import { matchesKey, type TUI } from "@linxiraos/pi-tui";
		...
		import("./crew/handlers/revise.ts")
			...
			.catch(err => {
				logFeedEvent(
					cwd,
					agentName,
					scope === "tree" ? "task.revise-tree" : "task.revise",
					task.id,
...
function collectMentionCandidates(prefix: string, state: MessengerState, dirs: Dirs, cwd: string): string[] {
	...
	if (matchesKey(data, "tab") || matchesKey(data, "shift+tab")) {
		...
		if (!cycling) {
			...
			viewState.mentionIndex =
				(viewState.mentionIndex + delta + viewState.mentionCandidates.length) % viewState.mentionCandidates.length;
		}
