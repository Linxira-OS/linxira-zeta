#!/bin/bash
set -u
cd "C:/Users/ETPau/Documents/GITHUB/zeta-sync-1821" || exit 1
mapfile -t HOPS < hops.txt
LAST=0; TOTAL=${#HOPS[@]}
STAGING=$(git ls-remote origin refs/heads/_sync-staging 2>/dev/null | cut -c1-40)
...
for t in $(seq 1 15); do
  git push origin sync/omp-release/v18.2.1 > /dev/null 2>&1 && break
  sleep 3
done
echo "FINAL: $(git ls-remote origin refs/heads/sync/omp-release/v18.2.1 | cut -c1-12)"
