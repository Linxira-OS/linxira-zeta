...
- **TTT agent 设置在默认配置中整段缺失**:`AgentSettings` 标了 `json:"agent,omitzero"` 而 `DefaultSettings()` 未赋该字段,导致新写配置里整块消失——Agent 菜单与两个 handoff 命令虽按 nil 即开生效,但用户没有任何键可以关掉它们。默认配置改为显式携带两字段;nil 仍表示"继承默认",旧配置不受影响。
