...
This runbook describes how `@linxiraos/pi-natives` produces `.node` addons, generated declarations, and compiled-binary embedded payloads, and how to debug loader/build failures.
...
It follows the architecture terms from `document/natives-architecture.md`:
...
>>>>>>> v18.2.11:docs/natives-build-release-debugging.md
=======
...
>>>>>>> v18.3.0:docs/natives-build-release-debugging.md

On non-PR events both jobs run on GitHub-hosted runners (`ubuntu-22.04`) with the `bazel-cache` action's disk-cache mode. `rust_validate` runs:
...
Bazel native jobs need no toolchain setup: bazelisk is on the GitHub images and baked into the kata runner image (the kata/cluster cache mode is retained for upstream-merge compatibility only and is dead code in this repo — CI is GitHub-hosted disk-cache only), while Bazel fetches Rust/zig/LLVM/xwin hermetically. The Windows ARM64 host build uses the Rust, Ninja, CMake, and Visual Studio ARM64 tools installed on `windows-11-arm`; `rust-toolchain.toml` selects the pinned nightly.
...
| rstest macro: "Cargo.toml not found" in a vendored test                                        | rstest verifies `Cargo.toml` exists in the manifest dir                                              | `compile_data = ["Cargo.toml"]` on the `rust_test`                                                                                   |
...
2. Loader resolves platform npm leaf-package candidates (`@linxiraos/pi-natives-<platform>-<arch>`, when resolvable), then package-local `native/` and executable-dir fallback candidates.
...
3. Runtime candidate order includes:
   ...
   - versioned cache dir,
   - legacy compiled-binary dir (`%LOCALAPPDATA%/zeta` on Windows, `~/.local/bin` elsewhere),
