...
- **Key decisions**:
  ...
  - Tree-wide sweeps post-merge (auto-merged files have no owner): 114+14 files `@oh-my-pi/*`→`@linxiraos/*` (omptype→pi-omptype, omp-stats→pi-stats, snapcompact→pi-snapcompact, pi-coding-agent→zeta); `.omp` fixture paths→`.zeta` except interop surfaces (`.omp-plugin`, `.omp-sessions`, `.omp/plugins` discovery, `omp.extensions`, `ompprurl`, `@omp-print-signal-`, `_omp_call`, `my.omp.sh`, `omp.sh/install`); `__omp_worker_*` selectors renamed to `__zeta_worker_*` to match the merged `worker-host.ts` validator (protocol constants follow the checker).
  ...
  - Brand: ζ title char kept; omfg-controller comment `.omp/rules`→`.zeta/rules`; brand-check 0 hits.
