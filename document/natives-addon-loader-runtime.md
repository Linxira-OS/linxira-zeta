...
This document covers the runtime loader shipped by `@linxiraos/pi-natives`: how `native/index.js` decides which `.node` file to require, how compiled-binary embedded payloads are extracted, and what startup failures report.
...
- release sentinel mismatch details when a loadable `.node` belongs to another `@linxiraos/pi-natives` version.

### Non-compiled startup failures

Normal package/runtime diagnostics include:

- reinstall hint (`bun install @linxiraos/pi-natives`),
