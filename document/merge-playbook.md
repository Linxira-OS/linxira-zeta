...
**结论 21（稳定步长 = 一个架构变更边界，不是固定 tag 数）**：本实验在 `dev/main`
上连合四个 tag（v18.2.6 / v18.2.11 / v18.3.0 / v18.3.1），冲突数依次 7 / 175 /
183 / 236，干跑预测与实际**四轮全对**（171/171、175/175、182 vs 183 差 1、236/236）。
冲突量与步长没有相关性：跨三个 tag 一次合（175）反而比逐 tag 合（145）只多 15%，
而含目录级增删的 v18.3.0（settings-schema 删除 + registry 新增 + internal-urls
集中化）单独成步时冲突只有 183。所以判据是**改动独立性**，不是 tag 数量或冲突数：

1. **常规轮**（无目录级增删）：可跨 2–3 个 tag 一次合，工作量按冲突面线性增长。
2. **架构变更轮**（目录级新增/删除、单体模块拆分为注册表一类）：**必须单独成步**。
   v18.3.0 的 hub 拆解和 v18.3.1 的 settings 注册表重构都属于这一类；把它们并进
   常规轮会让"上一轮遗留断点"与"本轮架构变更的破坏面"混在一起，无法归因——v18.3.0
   那次正是如此，多花的 227 个类型错误其实是上一轮遗留的。
3. 每轮之间保持 `check:ts` + 全部门禁绿，不要带断点进下一轮。
