...
The compiled macOS `zeta` binaries shipped on GitHub Releases can be signed with a
...
submission (see [#776](https://github.com/can1357/linxira-zeta/issues/776)).
...
>>>>>>> v18.2.11:docs/macos-signing-notarization.md
...
- `curl https://linxira-os.github.io/install | sh` — `curl` sets no quarantine bit, so
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/zeta-cli-darwin-arm64
```
