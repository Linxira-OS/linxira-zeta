...
This document defines the JS/TS contract between `@linxiraos/pi-natives` callers and the loaded N-API addon.
...
2. **Generated TypeScript declarations** (`native/index.d.ts`)
   - generated by napi-rs during `scripts/build-bindings.ts`;
...
Consumers in `packages/coding-agent` and `packages/tui` import directly from `@linxiraos/pi-natives`.
