...
import { createGatewayOpenToken, listHostOpenTargets, validateGatewayOpenTarget } from "../src/open-bridge.ts";
...
test("rejects forged tokens, unknown target IDs, and paths outside the workspace", () => {
	...
	try {
		...
		assert.equal(
			validateGatewayOpenTarget("editor:vscode", { ...authorized, token: "forged" }, paths.workspace, secret),
			null,
		);
